...
class TestTranslations(TestCase):
    ...
    def test_url_parse_inputs(self):
        ...
        self.assertEqual(
            result["inputs"]["inputFile"], "agave://test.system/test%20file.txt"
        )
        self.assertEqual(
            result["inputs"]["inputFiles"][0], "agave://test.system/test%20file%201.txt"
        )
        self.assertEqual(
            result["inputs"]["inputFiles"][1], "agave://test.system/test%20file%202.txt"
        )

        # Assert original object has not mutated
        self.assertEqual(
            self.job["inputs"]["inputFile"], "agave://test.system/test file.txt"
        )
...
class TestAgaveJWTAuth(TestCase):
    ...
    def test_login_user_agave_jwt(
        self, mock_decode_jwt, mock_get_jwt_payload, mock_login
    ):
    ...
    @override_settings(AGAVE_JWT_PUBKEY="pub-key==", AGAVE_JWT_HEADER="x_agave_header")
    @patch("portal.utils.jwt_auth.login")
    ...
    @patch(
        "portal.utils.jwt_auth._decode_jwt",
        return_value={"http://wso2.org/claims/fullname": "wma_prtl"},
    )
    @patch("portal.apps.auth.models.TapisOAuthToken.client", autospec=True)
    def test_agave_jwt_expired_token(
        self, mock_client, mock_decode_jwt, mock_get_jwt_payload, mock_login
    ):
