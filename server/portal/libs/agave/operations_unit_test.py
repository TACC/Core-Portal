...
from portal.libs.agave.operations import (
    listing,
    search,
    mkdir,
    move,
...
class TestOperations(TestCase):
    ...
    def test_search(self, mock_search, mock_listing):
        ...
        mock_search().query().filter().filter().filter().extra().execute.return_value = (
            mock_result
        )

        search_res = search(
            None, "test.system", "/path", query_string="query", hideTrash="True"
        )
        ...
        mock_search().query().filter.assert_called_with(
            ~Q("query_string", query="\\/.Trash\\/", fields=["path"])
        )
        mock_search().query().filter().filter.assert_called_with(
            "prefix", **{"path._exact": "path"}
        )
        mock_search().query().filter().filter().filter.assert_called_with(
            "term", **{"system._exact": "test.system"}
        )
        mock_search().query().filter().filter().filter().extra.assert_called_with(
            from_=int(0), size=int(100)
        )
        self.assertEqual(
            search_res,
            {
                "listing": [{"system": "test.system", "path": "/path/to/file"}],
                "reachedEnd": True,
                "count": 1,
            },
        )
    ...
    def test_mkdir(self, mock_indexer):
        ...
        client.files.mkdir.assert_called_with(
            systemId="test.system", path="/root/testfolder"
        )
    ...
    def test_make_public(self, mock_copy):
        ...
        mock_copy.assert_called_with(
            client, "test.system", "/path/to/src", "cloud.data", "/"
        )
