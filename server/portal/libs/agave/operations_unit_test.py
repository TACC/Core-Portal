...
class TestOperations(TestCase):
    ...
    def test_listing(self, mock_indexer):
        ...
        client.files.listFiles.assert_called_with(
            systemId="test.system", path="/path/to/file", pattern="", offset=1, limit=100, headers={"X-Tapis-Tracking-ID": ""}
        )

        mock_response_listing = [
            {
                "uuid": None,
                "system": "test.system",
                "type": "file",
                ...
                "length": 1,
                "lastModified": "2020-04-23T06:25:56Z",
                "_links": {"self": {"href": "tapis://cloud.data/path/to/file"}},
                "metadata": None,
            }
        ]
        ...
        self.assertEqual(ls, {"listing": mock_response_listing, "reachedEnd": True, "folder_metadata": None})
    ...
    def test_search(self, mock_search, mock_listing):
        ...
        mock_search().query().filter().filter().filter().extra().execute.return_value = mock_result

        search_res = search(None, "test.system", "/path", query_string="query", hideTrash="True")

        mock_search().query.assert_called_with(
            Q("query_string", query="query", fields=["name"], minimum_should_match="100%", default_operator="or")
            | Q("query_string", query="query", fields=["name._exact, name._pattern"], default_operator="and")
        )
        ...
        self.assertEqual(search_res, {"listing": [{"system": "test.system", "path": "/path/to/file"}], "reachedEnd": True, "count": 1})
    ...
    def test_mkdir(self, mock_indexer):
        ...
        mock_indexer.apply_async.assert_called_with(
            kwargs={"access_token": "my_access_token", "systemId": "test.system", "filePath": "/root", "recurse": False}
        )
    ...
    def test_rename(self, mock_move):
        ...
        mock_move.assert_called_with(
            client, src_system="test.system", src_path="/path/to/file", dest_system="test.system", dest_path="/path/to", file_name="newname", metadata=None
        )
    ...
    def test_move(self, mock_indexer):
        ...
        client.files.moveCopy.assert_called_with(
            systemId="test.system", path="/path/to/src", operation="MOVE", newPath="path/to/dest/src", headers={"X-Tapis-Tracking-ID": ""}
        )
    ...
    def test_copy(self, mock_indexer):
        ...
        client.files.moveCopy.assert_called_with(
            systemId="test.system", path="/path/to/src", operation="COPY", newPath="path/to/dest/src", headers={"X-Tapis-Tracking-ID": ""}
        )
    ...
    def test_make_public(self, mock_copy):
        ...
        mock_copy.assert_called_with(client, "test.system", "/path/to/src", "cloud.data", "/")
