...
class TestAgaveUtils(TestCase):
    ...
    def setUp(self):
        ...
        with open(os.path.join(agave_path, "files", "file.json")) as _file:
            self.agave_file = json.load(_file)

        with open(os.path.join(agave_path, "files", "directory.json")) as _file:
            self.agave_directory = json.load(_file)

        with open(os.path.join(agave_path, "files", "listing.json")) as _file:
            self.agave_listing = json.load(_file)

        with open(os.path.join(agave_path, "files", "file-listing.json")) as _file:
    ...
    def test_walk_levels(self):
        ...
        agave_dir = [obj for obj in self.agave_listing if obj["type"] == "dir" and obj["name"] != "."][0]
        ...
        for root, folders, files in AgaveUtils.walk_levels(self.magave, self.agave_listing[0]["system"], self.agave_listing[0]["path"]):
            levels_visited.append((root, folders, files))

        self.assertEqual(
            self.magave.files.listFiles.call_args_list,
            [
                call(systemId=self.agave_listing[0]["system"], path=self.agave_listing[0]["path"], offset=0, limit=100),
                call(systemId=agave_dir["system"], path=agave_dir["path"], offset=0, limit=100),
            ],
        )
        ...
        for index, level in enumerate(levels_visited):
            ...
            root = listing[0]["path"]
            folders = [f["path"] for f in listing if f["format"] == "folder" and f["name"] != "."]
            files = [f["path"] for f in listing if f["format"] != "folder"]
            self.assertEqual(root, level[0])
            self.assertEqual(folders, [f["path"] for f in level[1]])
            self.assertEqual(files, [f["path"] for f in level[2]])
