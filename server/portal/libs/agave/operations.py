...
def search(client, system, path="", offset=0, limit=100, query_string="", filter=None, **kwargs):
    ...
    ngram_query = Q(
        "query_string", query=query_string, fields=["name"], minimum_should_match="100%", default_operator="or"
    )
...
def delete(client, system, path, *args, **kwargs):
    ...
    return client.files.delete(
        systemId=system, path=path, headers={"X-Tapis-Tracking-ID": kwargs.get("tapis_tracking_id", "")}
    )
...
def trash(client, system, path, homeDir, metadata=None, *args, **kwargs):
    ...
    resp = move(
        client, system, path, system, f"{homeDir}/{settings.TAPIS_DEFAULT_TRASH_NAME}", file_name, metadata, **kwargs
    )
