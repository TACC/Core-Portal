...
def listing(client, system, path, offset=0, limit=100, *args, **kwargs):
    ...
    """
    pattern = (
        "regex:^(?!.Trash)" if "hideTrash" in kwargs and kwargs["hideTrash"] else ""
    )
    ...
    metadata_enabled = (
        settings.PORTAL_PROJECTS_ENABLE_METADATA and kwargs.get("scheme") == "projects"
    )
    ...
    try:
        ...
        for f in raw_listing:
            if not metadata_enabled:
                ...
                value = (
                    get_ordered_value(constants.FILE, file_obj.get("value"))
                    if file_obj
                    else None
                )
        ...
        logger.exception(
            f"Error parsing listing response from Tapis for system {system} and path {path}"
        )
    ...
    return {
        ...
        "folder_metadata": folder_entity_value,
    }
...
def search(
    client, system, path="", offset=0, limit=100, query_string="", filter=None, **kwargs
):
    """
...
def download(client, system, path, max_uses=3, lifetime=600, **kwargs):
    ...
    create_postit_result = client.files.createPostIt(
        systemId=system, path=path, allowedUses=max_uses, validSeconds=lifetime
    )
    ...
    **kwargs,
    ...
    if metadata is not None:
        if metadata.get("data_type") == "file":
            ...
            patch_entity_and_node(
                src_system, metadata, src_path.strip("/"), dest_path, file_name
            )
...
def copy(
    ...
    **kwargs,
    ...
    if metadata is not None:
        if metadata.get("data_type") == "file":
            patch_file_association(
                src_system, metadata, src_path, dest_path_full, file_name, "copy"
            )
    ...
    if src_system == dest_system:
        ...
        copy_response = client.files.createTransferTask(
            elements=[{"sourceURI": src_url, "destinationURI": dest_url}],
            headers={"X-Tapis-Tracking-ID": kwargs.get("tapis_tracking_id", "")},
        )
...
def trash(client, system, path, homeDir, metadata=None, *args, **kwargs):
    ...
    try:
        client.files.listFiles(
            systemId=system, path=f"{homeDir}/{settings.TAPIS_DEFAULT_TRASH_NAME}"
        )
    ...
    resp = move(
        ...
        path,
        system,
        f"{homeDir}/{settings.TAPIS_DEFAULT_TRASH_NAME}",
        file_name,
        metadata,
        **kwargs,
    )
...
def upload(client, system, path, uploaded_file, metadata=None, *args, **kwargs):
    ...
    uploaded_file.name = increment_file_name(
        listing=file_listing, file_name=uploaded_file.name
    )

    dest_path = os.path.join(path.strip("/"), uploaded_file.name)

    if (
        settings.PORTAL_PROJECTS_ENABLE_METADATA
        and metadata is not None
        and getattr(constants, metadata.get("data_type").upper(), None)
    ):

        parent_node = get_node_from_path(system, path)

        file_obj = create_file_obj(
            system, uploaded_file.name, uploaded_file.size, dest_path, metadata
        )
    ...
    upload_url = (
        f"{settings.TAPIS_TENANT_BASEURL}/v3/files/ops/{system}/{dest_path.lstrip('/')}"
    )
    headers = {
        "x-tapis-token": client.get_access_jwt(),
        "X-Tapis-Tracking-ID": kwargs.get("tapis_tracking_id", ""),
    }

    res = httpx.post(
        upload_url, headers=headers, files={"file": uploaded_file}, timeout=600
    )
...
def preview(client, system, path, max_uses=3, lifetime=600, **kwargs):
    ...
    if file_ext in settings.SUPPORTED_TEXT_PREVIEW_EXTS:
        ...
        [
            ext
            for ext in settings.SUPPORTED_BRAINMAP_PREVIEW_EXTS
...
def upload_file_metadata(
    client, system, path, file_name, file_size, metadata, **kwargs
):
