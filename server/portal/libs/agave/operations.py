...
from portal.apps.projects.workspace_operations.graph_operations import get_node_from_path, get_or_create_trash_entity, get_root_node
...
def detail(client, system, path, *args, **kwargs):
    ...
    """
    _listing = client.files.listFiles(
        systemId=system, path=urllib.parse.quote(path), offset=0, limit=1, headers={"X-Tapis-Tracking-ID": kwargs.get("tapis_tracking_id", "")}
    )
    f = _listing[0]
    listing_res = {
        "system": system,
...
def search(client, system, path="", offset=0, limit=100, query_string="", filter=None, **kwargs):
    ...
    ngram_query = Q("query_string", query=query_string, fields=["name"], minimum_should_match="100%", default_operator="or")
    match_query = Q("query_string", query=query_string, fields=["name._exact, name._pattern"], default_operator="and")
    ...
    return {"listing": hits, "count": res.hits.total.value, "reachedEnd": len(hits) < int(limit)}
...
def mkdir(client, system, path, dir_name, metadata=None, **kwargs):
    ...
    tapis_indexer.apply_async(
        kwargs={"access_token": client.access_token.access_token, "systemId": system, "filePath": path, "recurse": False},
    )
...
def move(client, src_system, src_path, dest_system, dest_path, file_name=None, metadata=None, **kwargs):
    ...
    if src_system == dest_system:
        move_result = client.files.moveCopy(
            systemId=src_system, path=src_path, operation="MOVE", newPath=dest_path_full, headers={"X-Tapis-Tracking-ID": kwargs.get("tapis_tracking_id", "")}
        )

    if os.path.dirname(src_path) != dest_path or src_path != dest_path:
        tapis_indexer.apply_async(
            kwargs={"access_token": client.access_token.access_token, "systemId": src_system, "filePath": os.path.dirname(src_path), "recurse": False},
            routing_key="indexing",
        )

    tapis_indexer.apply_async(
        kwargs={"access_token": client.access_token.access_token, "systemId": dest_system, "filePath": os.path.dirname(dest_path_full), "recurse": False},
        routing_key="indexing",
    )
    ...
    if file_info.dir:
        tapis_indexer.apply_async(
            kwargs={"access_token": client.access_token.access_token, "systemId": dest_system, "filePath": dest_path_full, "recurse": True},
            routing_key="indexing",
        )
...
@transaction.atomic
def copy(client, src_system, src_path, dest_system, dest_path, file_name=None, metadata=None, *args, **kwargs):
    ...
    if src_system == dest_system:
        copy_result = client.files.moveCopy(
            systemId=src_system, path=src_path, operation="COPY", newPath=dest_path_full, headers={"X-Tapis-Tracking-ID": kwargs.get("tapis_tracking_id", "")}
        )
    else:
        src_url = f"tapis://{src_system}/{src_path}"
        dest_url = f"tapis://{dest_system}/{dest_path_full}"

        copy_response = client.files.createTransferTask(
            elements=[{"sourceURI": src_url, "destinationURI": dest_url}], headers={"X-Tapis-Tracking-ID": kwargs.get("tapis_tracking_id", "")}
        )
    ...
    tapis_indexer.apply_async(
        kwargs={"access_token": client.access_token.access_token, "systemId": dest_system, "filePath": dest_path_full, "recurse": True}, routing_key="indexing"
    )
...
def delete(client, system, path, *args, **kwargs):
    ...
    return client.files.delete(systemId=system, path=path, headers={"X-Tapis-Tracking-ID": kwargs.get("tapis_tracking_id", "")})
...
def rename(client, system, path, new_name, metadata=None, *args, **kwargs):
    ...
    return move(client, src_system=system, src_path=path, dest_system=system, dest_path=new_path, file_name=new_name, metadata=metadata, **kwargs)
...
def trash(client, system, path, homeDir, metadata=None, *args, **kwargs):
    ...
    try:
        client.files.listFiles(systemId=system, path=f"{homeDir}/{settings.TAPIS_DEFAULT_TRASH_NAME}")
    ...
    resp = move(client, system, path, system, f"{homeDir}/{settings.TAPIS_DEFAULT_TRASH_NAME}", file_name, metadata, **kwargs)
...
def upload(client, system, path, uploaded_file, metadata=None, *args, **kwargs):
    ...
    headers = {"x-tapis-token": client.get_access_jwt(), "X-Tapis-Tracking-ID": kwargs.get("tapis_tracking_id", "")}

    res = httpx.post(upload_url, headers=headers, files={"file": uploaded_file}, timeout=600)
    res.raise_for_status()
    # NOTE: tapipy causing issues currently by reading file into memory, so not using it for uploads at the moment
...
def preview(client, system, path, max_uses=3, lifetime=600, **kwargs):
    ...
    postit = client.files.createPostIt(
        systemId=system, path=path, allowedUses=max_uses, validSeconds=lifetime, headers={"X-Tapis-Tracking-ID": kwargs.get("tapis_tracking_id", "")}
    )
