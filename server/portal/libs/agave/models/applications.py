...
class Application(BaseAgaveResource):
    ...
    PARALLELISM = namedtuple("Parallelism", _PARALLELISM)(SERIAL="SERIAL", PARALLEL="PARALLEL", PTHREAD="PTHREAD")

    _EXECUTION_TYPE = ["ATMOSPHERE", "HPC", "CONDOR", "CLI"]
    EXECUTION_TYPE = namedtuple("ExecutionType", _EXECUTION_TYPE)(ATMOSPHERE="ATMOSPHERE", HPC="HPC", CONDOR="CONDOR", CLI="CLI")
    ...
    def __init__(self, client, id=None, load=True, ignore_error=404, **kwargs):
        ...
        if id is not None and load:
            # try:
            wrapped = client.apps.get(appId=id)
    ...
    def permissions(self):
        ...
        if self.is_public:
            raise APIError('Cannot list permissions on public apps."is_public" must be "False".')
    ...
    def validate_parameters(self):
        ...
        if self.parameters is None:
            raise ValidationError("'parameters' should not be None")
    ...
    def validate_version(self):
        """Validate self.version"""
        if not self.version:
            raise ValidationError("'version' should not be empty")

    def validate_checkpointable(self):
        """Validate self.checkpointable"""
        if not isinstance(self.checkpointable, bool):
            raise ValidationError("'checkpointable' should be of type 'bool'")

    def validate_label(self):
        """Validate self.label"""
        if self.label is None:
            raise ValidationError("'label' should not be None")
    ...
    def validate_short_description(self):
        ...
        if self.short_description is None:
            raise ValidationError("'short_description' should not be None")
    ...
    def create(cls, client, app_def):
        ...
        if app_def.get("id") is not None:
            raise CreationError('Cannot specify "id" if creating application."id" must be "None".')
    ...
    def delete(self):
        ...
        """
        if self.id is None:
            raise DeletionError('Must specify "id" to delete application record')
        res = self._ac.apps.delete(appId=self.id)
        return res

    def update(self):
        """Update an application record."""
        self.validate()
        self._ac.apps.update(appId=self.id, body=self.to_dict())
    ...
    def save(self):
        ...
        """
        self.validate()
        self._ac.apps.add(body=self.to_dict())
    ...
    def clone(self, client, depl_path=None, exec_sys=None, depl_sys=None, name=None, ver=None):
        ...
        if self.last_modified is None:
            raise CreationError('Host app must already exist to be cloned."last_modified" must not be "None".')

        body = {"action": "clone", "deploymentPath": depl_path, "executionSystem": exec_sys, "deploymentSystem": depl_sys, "name": name, "version": ver}
    ...
    def publish(self, client):
        ...
        self.validate()
        if self.is_public:
            raise CreationError('Cannot publish public apps."is_public" must be "False".')
