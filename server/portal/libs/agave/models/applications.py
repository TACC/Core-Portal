...
class Application(BaseAgaveResource):
    ...
    PARALLELISM = namedtuple("Parallelism", _PARALLELISM)(
        SERIAL="SERIAL", PARALLEL="PARALLEL", PTHREAD="PTHREAD"
    )

    _EXECUTION_TYPE = ["ATMOSPHERE", "HPC", "CONDOR", "CLI"]
    EXECUTION_TYPE = namedtuple("ExecutionType", _EXECUTION_TYPE)(
        ATMOSPHERE="ATMOSPHERE", HPC="HPC", CONDOR="CONDOR", CLI="CLI"
    )
    ...
    def __init__(self, client, id=None, load=True, ignore_error=404, **kwargs):
        ...
        self.default_processors_per_node = getattr(
            self, "default_processors_per_node", None
        )
    ...
    def permissions(self):
        ...
        if self.is_public:
            raise APIError(
                "Cannot list permissions on public apps." '"is_public" must be "False".'
            )
    ...
    def validate_execution_type(self):
        ...
        if self.execution_type not in types:
            raise ValidationError(
                "'execution_type' should be one of: {types}".format(types=types)
            )
    ...
    def validate_parallelism(self):
        ...
        if self.parallelism not in types:
            raise ValidationError(
                "'parallelism' should be one of: {types}".format(types=types)
            )
    ...
    def create(cls, client, app_def):
        ...
        if app_def.get("id") is not None:
            raise CreationError(
                'Cannot specify "id" if creating application.' '"id" must be "None".'
            )
    ...
    def clone(
        self, client, depl_path=None, exec_sys=None, depl_sys=None, name=None, ver=None
    ):
    ...
    def publish(self, client):
        ...
        if self.is_public:
            raise CreationError(
                "Cannot publish public apps." '"is_public" must be "False".'
            )
