...
class Permission:
    ...
    def to_dict(self):
        """Dict representation"""
        return {"username": self.username, "recursive": self.recursive, "permission": self.value}
    ...
    def __repr__(self):
        ...
        return f"Permissions({self.username},recursive={self.recursive},read={self.read},write={self.write},execute={self.execute})"
...
class Permissions:
    ...
    def for_user(self, username):
        ...
        return Permission({"username": username, "recursive": False, "permission": {"read": False, "write": False, "execute": False}})
    ...
    def add(self, username, recursive=True, read=False, write=False, execute=False):  # pylint: disable=too-many-arguments
        ...
        if pems:
            ...
            pem = Permission({"username": username, "recursive": recursive, "permission": {"read": read, "write": write, "execute": execute}})
...
class FilePermissions(Permissions):
    ...
    def save(self):
        ...
        for pem in self.to_update:
            res = self._ac.files.updatePermissions(filePath=self.parent.path, systemId=self.parent.system, body=pem.to_dict())
...
class MetadataPermissions(Permissions):
    ...
    def save(self):
        """Save."""
        for pem in self.to_update:
            self._ac.meta.updateMetadataPermissions(uuid=self.parent.uuid, body=pem.to_dict())
...
class ApplicationPermissions(Permissions):
    ...
    def save(self):
        """Save."""
        for pem in self.to_update:
            res = self._ac.apps.updateApplicationPermissions(appId=self.parent.id, body=pem.to_dict())
