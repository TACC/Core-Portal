...
def mock_uploader(mocker):
    ...
    patched = mocker.patch("portal.libs.googledrive.operations.MediaIoBaseUpload")
        ...
        "listing": [
            {
                ...
                "_links": {"self": {"href": "http://webviewlink"}},
            }
        ],
        "nextPageToken": None,
        "reachedEnd": True,
...
def test_googledrive_listing(mock_googledrive_client, mock_googledrive_listing):
    from portal.libs.googledrive.operations import listing

    mock_googledrive_client.files().list().execute.return_value = mock_googledrive_listing

    test_listing = listing(mock_googledrive_client, "googledrive", "abcd", nextPageToken=None)
...
def test_googledrive_search(mock_googledrive_client, mock_googledrive_listing):
    from portal.libs.googledrive.operations import search

    mock_googledrive_client.files().list().execute.return_value = mock_googledrive_listing

    test_listing = search(mock_googledrive_client, "googledrive", "abcd", nextPageToken=None, query_string="testquery")
...
def test_upload(mock_googledrive_client, mock_uploader):
    ...
    mock_googledrive_client.files().create.assert_called_with(body={"name": "testfile", "parents": ["testpath"]}, media_body=mock_uploader())
...
def test_mkdir(mock_googledrive_client):
    ...
    mock_googledrive_client.files().create.assert_called_with(
        body={"name": "testdir", "parents": ["testid"], "mimeType": "application/vnd.google-apps.folder"},
...
def test_download(mock_googledrive_client, mock_downloader):
    ...
    mock_googledrive_client.files().get().execute.return_value = {"name": "testfile"}
...
def test_copy_file(mock_googledrive_client, mocker):
    ...
    copy(mock_googledrive_client, "googledrive", "src_id", "googledrive", "dest_id", "testfile", filetype="file")

    mock_transfer.assert_called_with(
        mock_googledrive_client, mock_googledrive_client, "googledrive", "googledrive", "googledrive", "googledrive", "src_id", "dest_id"
    )
...
def test_copy_dir(mock_googledrive_client, mocker):
    ...
    mock_transfer = mocker.patch("portal.libs.transfer.operations.transfer_folder")
    copy(mock_googledrive_client, "googledrive", "src_id", "googledrive", "dest_id", "testfile", filetype="dir")

    mock_transfer.assert_called_with(
        mock_googledrive_client, mock_googledrive_client, "googledrive", "googledrive", "googledrive", "googledrive", "src_id", "dest_id", "testfile"
    )
