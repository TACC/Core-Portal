...
def listing(client, system, path, offset=None, limit=100, nextPageToken=None, *args, **kwargs):
    ...
    listing_call = (
        client.files()
        .list(q=f"'{path}' in parents and trashed=False", fields=f"files({fields}), nextPageToken", pageSize=limit, pageToken=nextPageToken)
        .execute()
    )
    ...
    listing = list(
        map(
            lambda f: {
                ...
                "_links": {"self": {"href": f["webViewLink"]}},
            },
            listing,
        )
    )
...
def search(client, system, path, offset=None, limit=100, nextPageToken=None, query_string="", *args, **kwargs):
    if not path:
        path = "root"
    fields = "mimeType, name, id, modifiedTime, fileExtension, size, parents, webViewLink"
    listing_call = (
...
def upload(client, system, path, uploaded_file, *args, **kwargs):
    ...
    file_meta = {"name": os.path.basename(uploaded_file.name), "parents": [path]}
...
def mkdir(client, system, path, dir_name):
    if not path:
        path = "root"
    file_metadata = {"name": dir_name, "parents": [path], "mimeType": "application/vnd.google-apps.folder"}
    fields = "mimeType, name, id, modifiedTime, fileExtension, size, parents"
    newdir = client.files().create(body=file_metadata, fields=fields).execute()
...
def copy(client, src_system, src_path, dest_system, dest_path, file_name, filetype="file", dest_path_name="", *args):
    ...
    if filetype == "dir":
        transfer_folder(client, client, "googledrive", "googledrive", src_system, dest_system, src_path, dest_path, file_name)

    return {"nativeFormat": filetype, "name": dest_path_name, "path": os.path.join(dest_path_name, file_name), "systemId": dest_system}
