...
from portal.libs.elasticsearch.analyzers import file_analyzer, file_pattern_analyzer, path_analyzer, reverse_file_analyzer
...
class IndexedProject(Document):
    ...
    name = Text()
    host = Text()
    owner = Object(properties={"username": Keyword(), "firstName": Text(), "lastName": Text(), "email": Text()})
    updated = Date()
...
class IndexedFile(Document):
    ...
    name = Text(
        analyzer=file_analyzer, fields={"_exact": Keyword(), "_pattern": Text(analyzer=file_pattern_analyzer), "_reverse": Text(analyzer=reverse_file_analyzer)}
    )
    path = Text(
        fields={"_comps": Text(analyzer=path_analyzer), "_exact": Keyword(), "_reverse": Text(analyzer=reverse_file_analyzer)},
    )
    lastModified = Date()
    length = Long()
    format = Text()
    ...
    basePath = Text(fields={"_comps": Text(analyzer=path_analyzer), "_exact": Keyword()})
    lastUpdated = Date()
    pems = Object(
        properties={
            "username": Keyword(),
            "recursive": Boolean(),
            "permission": Object(properties={"read": Boolean(), "write": Boolean(), "execute": Boolean()}),
        }
    )
...
class IndexedAllocation(Document):
    ...
    def from_username(cls, username):
        ...
        elasticsearch.exceptions.NotFoundError
        """
        es_client = Elasticsearch(hosts=settings.ES_HOSTS, http_auth=settings.ES_AUTH, timeout=10)
