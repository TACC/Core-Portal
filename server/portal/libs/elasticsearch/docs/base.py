...
class IndexedFile(Document):
    ...
    basePath = Text(
        fields={"_comps": Text(analyzer=path_analyzer), "_exact": Keyword()}
    )
    lastUpdated = Date()
    pems = Object(
        properties={
            ...
            "recursive": Boolean(),
            "permission": Object(
                properties={"read": Boolean(), "write": Boolean(), "execute": Boolean()}
            ),
        }
    )
...
class IndexedAllocation(Document):
    ...
    def from_username(cls, username):
        ...
        es_client = Elasticsearch(
            hosts=settings.ES_HOSTS, http_auth=settings.ES_AUTH, timeout=10
        )
