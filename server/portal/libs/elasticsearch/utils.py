...
def delete_recursive(system, path):
    ...
    for group in grouper(hits, 100):
        ...
        ops = map(lambda hit: {"_index": idx, "_id": hit.meta.id, "_op_type": "delete"}, filtered_group)
        bulk(client, ops)
...
def index_listing(files):
    ...
    for _file in files:
        ...
        ops.append({"_index": idx, "_id": file_uuid, "doc": file_dict, "_op_type": "update", "doc_as_upsert": True})
...
def index_project_listing(projects):
    ...
    for _project in projects:
        ...
        ops.append(
            {"_index": idx, "_id": project_uuid, "doc": project_dict, "_op_type": "update", "doc_as_upsert": True}
        )
