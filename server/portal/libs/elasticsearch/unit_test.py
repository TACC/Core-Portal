...
from portal.libs.elasticsearch.utils import delete_recursive, file_uuid_sha256, grouper, index_level, index_listing, walk_children
...
class TestESUtils(TestCase):
    ...
    def test_delete_recursive(self, mock_conn, mock_bulk, mock_children):
        ...
        test_op = {"_index": "test-staging-files", "_id": "ABCDEF", "_op_type": "delete"}

        delete_recursive("test.system", "/test/file")
        mock_children.assert_called_once_with("test.system", "/test/file", include_parent=True, recurse=True)
            ...
            [
                {
                    ...
                    "doc": {"system": "test.system", "name": "file1", "path": "/test/file1", "lastUpdated": "TIME_NOW", "basePath": "/test"},
                    "_op_type": "update",
                    "doc_as_upsert": True,
                }
            ],
