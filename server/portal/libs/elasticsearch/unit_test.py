...
from portal.libs.elasticsearch.indexes import (
    setup_files_index,
...
class TestESUtils(TestCase):
    ...
    def test_walk_children(self, mock_search):
        ...
        children = walk_children(
            "test.system", "/file/path", include_parent=True, recurse=True
        )
        next(children)
        mock_search().filter().filter.assert_called_with(
            Q({"prefix": {"basePath._exact": "/file/path"}})
            | Q({"term": {"path._exact": "/file/path"}})
        )

        children = walk_children(
            "test.system", "/file/path", include_parent=True, recurse=False
        )
        next(children)
        mock_search().filter().filter.assert_called_with(
            Q({"term": {"basePath._exact": "/file/path"}})
            | Q({"term": {"path._exact": "/file/path"}})
        )

        children = walk_children(
            "test.system", "/file/path", include_parent=False, recurse=True
        )
        next(children)
        mock_search().filter().filter.assert_called_with(
            Q({"prefix": {"basePath._exact": "/file/path"}})
        ...
        children = walk_children(
            "test.system", "/file/path", include_parent=False, recurse=False
        )
        next(children)
        mock_search().filter().filter.assert_called_with(
            Q({"term": {"basePath._exact": "/file/path"}})
    ...
    def test_delete_recursive(self, mock_conn, mock_bulk, mock_children):
        ...
        mock_children.assert_called_once_with(
            "test.system", "/test/file", include_parent=True, recurse=True
        )
