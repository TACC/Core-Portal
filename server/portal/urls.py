...
urlpatterns = [
    ...
    path("googledrive-privacy-policy/", include("portal.apps.googledrive_integration.urls", namespace="googledrive-privacy-policy")),
...
if settings.WORKBENCH_SETTINGS.get("hasCustomEndpoints"):
    urlpatterns.append(
        path(f"api/{settings.PORTAL_NAMESPACE.lower()}/", include(f"portal.apps._custom.{settings.PORTAL_NAMESPACE.lower()}.urls", namespace="custom"))
    )
# internal docs
