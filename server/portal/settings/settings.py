...
TEMPLATES = [
    {
        ...
        "DIRS": [os.path.join(BASE_DIR, "templates"), os.path.join(BASE_DIR, "../../client/dist")],
        # 'APP_DIRS': True,
        "OPTIONS": {
            "context_processors": [
...
AUTHENTICATION_BACKENDS = ["portal.apps.auth.backends.TapisOAuthBackend", "django.contrib.auth.backends.ModelBackend"]
...
LOGGING = {
    ...
    "formatters": {
        "default": {"format": "[DJANGO] %(levelname)s %(asctime)s UTC %(module)s %(name)s.%(funcName)s:%(lineno)s: %(message)s"},
        "tapis": {"format": "[TAPIS] %(levelname)s %(asctime)s UTC %(module)s %(name)s.%(funcName)s:%(lineno)s: %(message)s"},
        "metrics": {
    ...
    "handlers": {
        ...
        "metrics": {"level": "INFO", "class": "logging.StreamHandler", "formatter": "metrics", "filters": ["portalFilter"]},
    },
    "loggers": {
        "django": {
...
CELERY_BROKER_URL = "".join(
    [_BROKER_URL_PROTOCOL, _BROKER_URL_USERNAME, ":", _BROKER_URL_PWD, "@", _BROKER_URL_HOST, ":", _BROKER_URL_PORT, "/", _BROKER_URL_VHOST]
)
...
CELERY_RESULT_BACKEND = "".join([_RESULT_BACKEND_PROTOCOL, _RESULT_BACKEND_HOST, ":", _RESULT_BACKEND_PORT, "/", _RESULT_BACKEND_DB])

CELERY_ACCEPT_CONTENT = ["json"]
CELERY_TASK_SERIALIZER = "json"
CELERY_RESULT_SERIALIZER = "json"
CELERYD_HIJACK_ROOT_LOGGER = False
CELERYD_LOG_FORMAT = "[DJANGO] $(processName)s %(levelname)s %(asctime)s %(module)s %(name)s.%(funcName)s:%(lineno)s: %(message)s"
...
CELERY_QUEUES = (
    ...
    Queue("files", Exchange("files"), routing_key="files", queue_arguments={"x-max-priority": 10}),
    # Use to queue tasks which mainly call external APIs
    Queue("api", Exchange("api"), routing_key="api", queue_arguments={"x-max-priority": 10}),
    # Use to queue tasks handling onboarding
    Queue("onboard", Exchange("onboard"), routing_key="onboard", queue_arguments={"x-max-priority": 10}),
)
...
TACC_EXEC_SYSTEMS = {
    ...
    "stampede3": {"work_dir": "/work2/{}", "scratch_dir": "/scratch/{}", "home_dir": "/home1/{}"},
    "frontera": {"work_dir": "/work2/{}", "scratch_dir": "HOST_EVAL(SCRATCH)", "home_dir": "/home1/{}"},
    "ls6": {"work_dir": "/work/{}", "scratch_dir": "/scratch/{}", "home_dir": "/home1/{}"},
    "vista": {"work_dir": "/work/{}", "scratch_dir": "/scratch/{}", "home_dir": "/home1/{}"},
}
...
PORTAL_DATAFILES_STORAGE_SYSTEMS = getattr(settings_custom, "_PORTAL_DATAFILES_STORAGE_SYSTEMS", [])
...
PORTAL_PROJECTS_REVIEW_SYSTEM_PREFIX = getattr(settings_custom, "_PORTAL_PROJECTS_REVIEW_SYSTEM_PREFIX", None)

PORTAL_PROJECTS_REVIEW_ROOT_DIR = getattr(settings_custom, "_PORTAL_PROJECTS_REVIEW_ROOT_DIR", None)

PORTAL_PROJECTS_ROOT_REVIEW_SYSTEM_NAME = getattr(settings_custom, "_PORTAL_PROJECTS_ROOT_REVIEW_SYSTEM_NAME", None)

PORTAL_PROJECTS_PUBLISHED_SYSTEM_PREFIX = getattr(settings_custom, "_PORTAL_PROJECTS_PUBLISHED_SYSTEM_PREFIX", None)

PORTAL_PROJECTS_PUBLISHED_ROOT_DIR = getattr(settings_custom, "_PORTAL_PROJECTS_PUBLISHED_ROOT_DIR", None)

PORTAL_PROJECTS_PUBLISHED_ROOT_SYSTEM_NAME = getattr(settings_custom, "_PORTAL_PROJECTS_PUBLISHED_ROOT_SYSTEM_NAME", None)
...
PORTAL_PUBLICATION_RANCH_SYSTEM_ID = getattr(settings_custom, "_PORTAL_PUBLICATION_RANCH_SYSTEM_ID", None)

ALLOCATIONS_TO_EXCLUDE = getattr(settings_custom, "_ALLOCATIONS_TO_EXCLUDE", ["DesignSafe-DCV", "DesignSafe-Corral"])
...
VANITY_BASE_URL = getattr(
    ...
    getattr(
        settings_custom,
        "_WH_BASE_URL",
        (f"https://{os.environ.get('NGINX_SERVER_NAME')}" if os.environ.get("NGINX_SERVER_NAME") else ""),
    ),
)
