...
class ApiException(RequestException):
    ...
    This class is meant to extend the basic :class:`~requests.exceptions.RequestException` and add
     the ability to add extra information.

    .. rubric:: Rationale

    It is better to raise custom exceptions when implementing an API to easily differentiate
     exceptions raise by our code and those exceptions raised by external libraries.
    By adding the ability to add extra information we can send more context to the logger.

    """

    def __init__(self, message=None, status=None, extra=None, *args, **kwargs):
        """Custom exception based on :class:`~requests.exceptions.RequestException`
        ...
        >>>     raise ApiException("Message", request=err.request, response=e.response)
        """
