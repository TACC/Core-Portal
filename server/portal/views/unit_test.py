...
def api_method_mock(mocker):
    ...
    """
    workbench_state = mocker.patch(
        "portal.apps.system_monitor.views.SysmonDataView.get"
    )
...
def test_connectionerror_httperror_no_response_in_exception(
    ExceptionClass, client, api_method_mock
):
...
def test_connectionerror_httperror_with_response(
    ExceptionClass, status_code, client, api_method_mock
):
    ...
    test_response._content = json.dumps({"message": "Custom error message"}).encode(
        "utf-8"
    )
...
@pytest.mark.parametrize("status_code", [403, 404, NON_403_404])
def test_connectionerror_httperror_non_json_content(
    ExceptionClass, status_code, client, api_method_mock
):
    test_response = requests.Response()
