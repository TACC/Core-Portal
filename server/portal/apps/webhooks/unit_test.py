...
class TestJobsWebhookView(TransactionTestCase):
    ...
    def test_webhook_job_post(self, mock_validate_tapis_job):
        ...
        response = self.client.post(
            reverse("webhooks:jobs_wh_handler"), json.dumps(job_notification_event), content_type="application/json"
        )
    ...
    def test_webhook_job_post_invalid_state(self, mock_validate_tapis_job):
        ...
        response = self.client.post(
            reverse("webhooks:jobs_wh_handler"), json.dumps(job_event), content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
