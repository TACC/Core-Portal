...
class TestValidateTapisJob(TestCase):
    def setUp(self):
        ...
        self.user_model_patcher = patch("portal.apps.webhooks.views.get_user_model", return_value=mock_user_model)
...
class TestJobsWebhookView(TransactionTestCase):
    ...
    def test_webhook_job_post(self, mock_validate_tapis_job):
        ...
        response = self.client.post(reverse("webhooks:jobs_wh_handler"), json.dumps(job_notification_event), content_type="application/json")
...
class TestInteractiveWebhookView(TestCase):
    ...
    def test_unsupported_event_type(self):
        response = self.client.post(
            reverse("webhooks:interactive_wh_handler"), urlencode({"event_type": "DUMMY"}), content_type="application/x-www-form-urlencoded"
        )
    ...
    def test_webhook_web_post(self):
        ...
        response = self.client.post(reverse("webhooks:interactive_wh_handler"), urlencode(self.web_event), content_type="application/x-www-form-urlencoded")
