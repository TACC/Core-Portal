...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--cleanup",
            help="Remove documents after swapping aliases to save space.",
            default=False,
            action="store_true",
        )
        parser.add_argument(
            "--swap-only",
            help="Only swap index aliases without reindexing.",
            default=False,
            action="store_true",
        )
    ...
    def handle(self, *args, **options):
        es_client = elasticsearch.Elasticsearch(
            [{"host": settings.ES_HOSTS, "http_auth": settings.ES_AUTH}], timeout=60
        )
        ...
        try:
            default_index_name = (
                Index(default_index_alias, using=es_client).get_alias().keys()[0]
            )
            reindex_index_name = (
                Index(reindex_index_alias, using=es_client).get_alias().keys()[0]
            )
        except Exception:
            self.stdout.write(
        ...
        if not swap_only:
            ...
            elasticsearch.helpers.reindex(
                es_client, default_index_name, reindex_index_name
            )
