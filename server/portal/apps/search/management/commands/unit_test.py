...
class TestSwapReindex(TestCase):
    def setUp(self):
        self.patch_setup = patch("portal.apps.search.management.commands.reindex_files.setup_files_index")
        self.patch_connections = patch("portal.apps.search.management.commands.reindex_files.connections")
        self.patch_elasticsearch = patch("portal.apps.search.management.commands.reindex_files.elasticsearch")
    ...
    @patch("portal.apps.search.management.commands.reindex_files.input")
    def test_raises_when_user_does_not_proceed(self, mock_input):
        mock_input.return_value = "n"

        with self.assertRaises(SystemExit):
            call_command("reindex_files")

    @patch("portal.apps.search.management.commands.reindex_files.Index")
    @patch("portal.apps.search.management.commands.reindex_files.input")
    ...
    def test_cleanup(self, mock_input, mock_index):
        ...
        call_command("reindex_files", **opts)
