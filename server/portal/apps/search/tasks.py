...
@shared_task(
    bind=True, max_retries=3, queue="indexing", retry_backoff=True, rate_limit="12/m"
)
def tapis_indexer(
    self,
    systemId,
    ...
    try:
        ...
        logger.error(
            "Error walking files under system {} and path {}".format(systemId, filePath)
        )
...
def index_community_data(self, reindex=False):
    for sys in settings.PORTAL_DATAFILES_STORAGE_SYSTEMS:
        if sys["api"] == "tapis" and sys["scheme"] in ["community", "public"]:
            logger.info(
                "INDEXING {} SYSTEM with file path {}".format(
                    sys["name"], sys.get("homeDir", "/")
                )
            )
            tapis_indexer.apply_async(
                args=[sys["system"]],
                kwargs={"filePath": sys.get("homeDir", "/"), "reindex": reindex},
            )
