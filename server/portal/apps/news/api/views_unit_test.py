...
@patch('portal.apps.news.api.views.requests.get')
def test_api_news_success_without_sanitize(mock_get, client, authenticated_user):
    ...
    assert body['response'][0]['content'] == '<p>Hello <b>world</b></p>'
    assert body['response'][0]['updates'][0]['content'] == '<div>Update</div>'
...
def test_api_news_success_with_sanitize(mock_get, client, authenticated_user):
    ...
    body = response.json()
    assert body['status'] == 200
    assert body['response'][0]['content'] == 'Hello world'
    assert body['response'][0]['updates'][0]['content'] == 'Update'
