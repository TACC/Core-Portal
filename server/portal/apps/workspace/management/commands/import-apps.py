...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "-n", "--names", type=str, help="Portal app names to import"
        )
        parser.add_argument(
            "-c", "--clean", action="store_true", help="Remove nonexistant apps"
        )
    ...
    def clean(self):
        ...
        for app in portal_apps:
            try:
                ...
                logger.info(
                    "App not found. id: {} and version: {}:. Deleting...".format(
                        app.appId, app.version or "None"
                    )
                )
                app.delete()
    ...
    def import_apps(self, portal_names):
        ...
        query = [
            "(enabled = TRUE) AND",
            f"((tags IN ('portalName: {portal_names[0]}'))",
        ]
        for portal_name in portal_names[1:]:
            query.append(f"OR (tags IN ('portalName: {portal_name}'))")
        query.append(")")

        data = client.apps.searchAppsRequestBody(
            search=query, select="id,notes,version"
        )
        for app in data:
            try:
