...
class TestAppsApiViews(TestCase):
    ...
    def setUp(self):
        ...
        with open(os.path.join(agave_path, "systems", "execution.json")) as _file:
            self.agave_execution = json.load(_file)

        with open(
            os.path.join(settings.BASE_DIR, "fixtures", "job-submission.json")
        ) as f:
        ...
        with open(
            os.path.join(settings.BASE_DIR, "fixtures", "tas", "tas_user.json")
        ) as f:
