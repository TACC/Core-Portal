...
class AppTrayEntry(models.Model):
    ...
    html = models.TextField(help_text="HTML definition to display when Application is loaded", default="", blank=True)
    available = models.BooleanField(help_text="App visibility in app tray", default=True)

    category = models.ForeignKey(AppTrayCategory, related_name="+", help_text="The App Category for this app entry", on_delete=models.CASCADE)

    def __str__(self):
        if self.appType == "html":
            return "%s: %s (HTML)" % (self.label, self.appId)
        return "%s%s%s" % (f"{self.label}: " if self.label else "", self.appId, f"-{self.version}" if self.version else "")
