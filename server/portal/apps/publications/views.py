    ...
    def get(self, request, project_id: str):
        ...
        if project_id:
            try:
                ...
                publication_requests = PublicationRequest.objects.filter(models.Q(source_project=project) | models.Q(review_project=project))
...
class PublicationListingView(BaseApiView):
    def get(self, request):
        ...
        if query_string:
            ...
            term_query = Q({"term": {"nodes.value.projectId.keyword": query_string.replace("/", "\\/")}})
            ...
            if hits:
                publications = Publication.objects.filter(project_id__in=hits, is_published=True).defer("tree").order_by("-created")
            else:
...
class PublicationVersionView(BaseApiView):
    def post(self, request):
        ...
        try:
            ...
            publish_project.apply_async(kwargs={"project_id": project_id, "version": version})
