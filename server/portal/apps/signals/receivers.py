...
def portal_event_callback(sender, **kwargs):
    ...
    if users:
        for user in users:
            async_to_sync(channel_layer.group_send)(str(user.id), {"type": "portal_notification", "body": data})
    else:
        async_to_sync(channel_layer.group_send)("portal_events", {"type": "portal_notification", "body": data})
...
def send_notification_ws(sender, instance, created, **kwargs):
    ...
    try:
        ...
        async_to_sync(channel_layer.group_send)(str(user.id), {"type": "portal_notification", "body": instance_dict})
    except Exception:
        logger.exception("Exception sending message to channel: portal_notification", extra=instance.to_dict())
    return
...
def send_setup_event(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.exception("Exception sending message to channel: portal_notification", extra=setup_event.to_dict())
    return
