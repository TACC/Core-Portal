...
def system_status(scope="module"):
    with open(
        os.path.join(settings.BASE_DIR, "fixtures/system_monitor/index.json")
    ) as f:
        yield json.load(f)
...
def system_status_missing_frontera(scope="module"):
    with open(
        os.path.join(
            settings.BASE_DIR, "fixtures/system_monitor/index_missing_frontera.json"
        )
    ) as f:
        yield json.load(f)
...
@pytest.mark.django_db()
def test_system_monitor_when_missing_system(
    client, settings, requests_mock, system_status_missing_frontera
):
    settings.SYSTEM_MONITOR_DISPLAY_LIST = ["Frontera"]
...
@pytest.mark.django_db()
def test_system_monitor_when_display_list_is_empty(
    client, settings, requests_mock, system_status
):
