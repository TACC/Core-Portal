    ...
    def get(self, request, system_name=None):
        ...
        if system_name:
            system_json = requests.get(
                f"{settings.SYSTEM_MONITOR_URL}{system_name}"
            ).json()
            ...
            for sys in requested_systems:
                if sys not in systems_json:
                    logger.info(
                        "System information for {} is missing. Assuming not operational status.".format(
                            sys
                        )
                    )
                ...
                try:
                    ...
                    logger.exception(
                        "Problem gather system information for {}: Assuming not operational status".format(
                            sys
                        )
                    )
                    systems.append(_get_unoperational_system(sys))
...
class System:
    ...
    def is_up(self):
        return (
            self.online
            and self.reachable
            and not (self.queues_down or self.in_maintenance)
        )
