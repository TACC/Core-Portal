...
def mock_rtutil(mocker, mock_rt):
    mocker.patch("portal.apps.tickets.utils.rtUtil.DjangoRt", return_value=mock_rt)
    yield mock_rt
...
@pytest.fixture
def get_user(mocker):
    mock = mocker.patch("portal.apps.request_access.api.views.TASClient.get_user")
...
def test_request_access_wrong_user_password(client, regular_user, get_authenticate_error):
    response = client.post(
        "/api/request-access/", data={"problem_description": "This is the problem description", "username": "testUsername", "password": "testPassword"}
    )
    assert response.status_code == 401


@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_request_access(client, regular_user, mock_rtutil, get_authenticate, get_user):
    response = client.post("/api/request-access/", data={"problem_description": "problem_description", "username": "testUsername", "password": "testPassword"})
