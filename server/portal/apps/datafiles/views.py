...
from portal.apps.datafiles.handlers.tapis_handlers import (
    tapis_get_handler,
    tapis_put_handler,
    tapis_post_handler,
)
...
from portal.apps.datafiles.utils import (
    evaluate_datafiles_storage_systems,
    get_user_storage_systems,
)
        ...
        if request.user.is_authenticated:
            ...
            if not portal_systems:
                ...
                default_system = (
                    ...
                    or settings.PORTAL_DATAFILES_STORAGE_SYSTEMS[0]
                )
            ...
            response["system_list"] = [
                ...
                or sys["system"] == settings.PORTAL_PROJECTS_PUBLISHED_ROOT_SYSTEM_NAME
            ]
...
class TapisFilesView(BaseApiView):
    ...
    def get(self, request, operation=None, scheme=None, system=None, path="/"):
        try:
            ...
            if (
                public_sys
                and public_sys["system"] == system
                and path.startswith(public_sys["homeDir"].strip("/"))
            ):
                client = service_account()
            elif (
                system
    ...
    def put(
        self, request, operation=None, scheme=None, handler=None, system=None, path="/"
    ):
...
class GoogleDriveFilesView(BaseApiView):
    ...
    def put(
        ...
        try:
            response = googledrive_put_handler(
                client, scheme, system, path, operation, body=body
            )
        except Exception as exc:
            operation in NOTIFY_ACTIONS and notify(
                request.user.username, operation, "error", {}
...
def get_client(user, api, system=None):
    ...
    if (
        api in ("tapis", "shared")
        and is_project_system(system)
        and check_project_admin_group(user)
    ):
        return service_account()
...
class TransferFilesView(BaseApiView):
    def put(self, request, filetype):
        ...
        dest_client = get_client(
            request.user, body["dest_api"], body.get("dest_system")
        )
        METRICS.info(
            "Data Files",
            extra={
...
class LinkView(BaseApiView):
    def create_postit(self, request, scheme, system, path):
        ...
        Link.objects.create(
            ...
            expiration=(
                dateutil.parser.parse(postit.expiration) if postit.expiration else None
            ),
        )
    ...
    def post(self, request, scheme, system, path):
        ...
        try:
            ...
            return JsonResponse(
                {"data": postit["data"], "expiration": postit["expiration"]}
            )
