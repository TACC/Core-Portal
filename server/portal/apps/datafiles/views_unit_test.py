...
def test_tapis_file_view_get_is_logged_for_metrics(
    mock_indexer, client, authenticated_user, mock_tapis_client, tapis_file_listing_mock, logging_metric_mock
):
...
def test_tapis_file_view_put_is_logged_for_metrics(
    ...
    response = client.put(
        "/api/datafiles/tapis/move/private/frontera.home.username/test.txt/", content_type="application/json", data=body
    )
    assert response.status_code == 200
...
def test_tapis_file_view_post_is_logged_for_metrics(
    ...
    response = client.post(
        "/api/datafiles/tapis/upload/private/frontera.home.username/", data={"uploaded_file": text_file_fixture}
    )
...
def test_tapis_file_view_preview_supported_non_text_files(
    client, authenticated_user, mock_tapis_client, agave_file_listing_mock, EXTENSION, TYPE
):
...
def test_tapis_file_view_preview_text_file(
    client, authenticated_user, mock_tapis_client, agave_file_listing_mock, requests_mock
):
    mock_tapis_client.files.listFiles.return_value = [TapisResult(**f) for f in agave_file_listing_mock]
    mock_tapis_client.files.createPostIt.return_value = TapisResult(redeemUrl=POSTIT_HREF, expiration=None)
    ...
    assert response.status_code == 200
    assert response.json() == {
        "data": {"href": POSTIT_HREF, "fileType": "text", "content": "file content", "error": None}
    }
...
def test_tapis_file_view_preview_other_text_file(
    ...
    assert response.status_code == 200
    assert response.json() == {
        "data": {"href": POSTIT_HREF, "fileType": "other", "content": "file content", "error": None}
    }
