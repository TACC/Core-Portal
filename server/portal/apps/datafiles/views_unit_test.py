...
def postits_create(mock_tapis_client):
    ...
    mock_tapis_client.files.createPostIt.return_value = TapisResult(
        redeemUrl="https://tenant/uuid", expiration=None
    )
...
def test_get_tapis_client_uses_service_account_for_project_admin(
    authenticated_user, mocker
):
...
def test_get_no_allocation(
    client, authenticated_user, mocker, monkeypatch, mock_tapis_client
):
    ...
    monkeypatch.setattr(
        mock_error,
        "response",
        MagicMock(json=MagicMock(return_value={}), status_code=500),
    )
    ...
    mock_tapis_client.systems.getSystem.return_value = TapisResult(
        host="frontera.tacc.utexas.edu", notes={}
    )

    response = client.get(
        "/api/datafiles/tapis/listing/private/frontera.home.username/"
    )
    assert response.status_code == 403


def test_ignore_missing_corral(
    client, authenticated_user, mocker, monkeypatch, mock_tapis_client
):
    mock_tapis_get = mocker.patch("portal.apps.datafiles.views.tapis_get_handler")
    mock_error = InternalServerError()
...
def test_get_requires_push_keys(
    client, authenticated_user, mocker, monkeypatch, mock_tapis_client
):
    mock_tapis_get = mocker.patch("portal.apps.datafiles.views.tapis_get_handler")
...
def test_link_post(postits_create, authenticated_user, client):
    ...
    postits_create.assert_called_with(
        systemId="system", path="path", allowedUses=-1, validSeconds=31536000
    )
...
def test_get_system(
    client, authenticated_user, mock_tapis_client, agave_storage_system_mock
):
    mock_tapis_client.systems.getSystem.return_value = TapisResult(
...
def test_get_system_forbidden(
    client, regular_user, mock_tapis_client, agave_storage_system_mock
):
...
def test_tapis_file_view_get_is_logged_for_metrics(
    mock_indexer,
    client,
    authenticated_user,
    mock_tapis_client,
...
@patch("portal.libs.agave.operations.tapis_indexer")
def test_tapis_file_view_put_is_logged_for_metrics(
    mock_indexer,
    client,
    authenticated_user,
    mock_tapis_client,
...
def test_tapis_file_view_put_is_logged_for_metrics_exception(
    mock_put_handler, mock_indexer, client, authenticated_user, mock_tapis_client
):
    mock_put_handler.side_effect = Exception(
        "Exception in Metrics info or Tapis Put Handler views.py:142"
    )
    body = {"dest_path": "/testfol", "dest_system": "frontera.home.username"}
    response = client.put(
        "/api/datafiles/tapis/move/private/frontera.home.username/test.txt/",
...
def test_tapis_file_view_post_is_logged_for_metrics(
    ...
    response = client.post(
        "/api/datafiles/tapis/upload/private/frontera.home.username/",
...
def test_tapis_file_view_post_is_logged_for_metrics_exception(
    ...
    mock_post_handler.side_effect = Exception(
        "Exception in Metrics info or Tapis Put Handler views.py:175"
    )
    mock_tapis_client.files.insert.return_value = tapis_file_mock
...
def test_tapis_file_view_preview_supported_non_text_files(
    client,
    authenticated_user,
    mock_tapis_client,
    agave_file_listing_mock,
    EXTENSION,
    TYPE,
):
    mock_tapis_client.files.listFiles.return_value = [
        TapisResult(**f) for f in agave_file_listing_mock
    ]
    mock_tapis_client.files.createPostIt.return_value = TapisResult(
        redeemUrl=POSTIT_HREF, expiration=None
    )
    response = client.put(
        "/api/datafiles/tapis/preview/private/frontera.home.username/test_text.{}/".format(
            EXTENSION
        ),
        content_type="application/json",
    ...
    assert response.status_code == 200
    assert response.json() == {
        "data": {"href": href, "fileType": TYPE, "content": None, "error": None}
    }


def test_tapis_file_view_preview_text_file(
    client,
    authenticated_user,
    mock_tapis_client,
    agave_file_listing_mock,
...
def test_tapis_file_view_preview_other_text_file(
    client,
    authenticated_user,
    mock_tapis_client,
    agave_file_listing_mock,
...
def test_tapis_file_view_preview_unsupported_file(
    client,
    authenticated_user,
    mock_tapis_client,
    agave_file_listing_mock,
    ...
    requests_mock.get(POSTIT_HREF, text="file content")
    response = client.put(
        "/api/datafiles/tapis/preview/private/frontera.home.username/test.html/",
        content_type="application/json",
        data={"href": "https//tapis.example/href"},
    )
    assert response.status_code == 200
...
def test_tapis_file_view_preview_large_file(
    client,
    authenticated_user,
    mock_tapis_client,
    agave_file_listing_mock,
    ...
    requests_mock.get(POSTIT_HREF, text="file content")
    response = client.put(
        "/api/datafiles/tapis/preview/private/frontera.home.username/test.log/",
        content_type="application/json",
        data={"href": "https//tapis.example/href"},
    )
    assert response.status_code == 200
