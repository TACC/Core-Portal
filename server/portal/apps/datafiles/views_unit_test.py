...
def postits_create(mock_tapis_client):
    ...
    mock_tapis_client.files.createPostIt.return_value = TapisResult(redeemUrl="https://tenant/uuid", expiration=None)
...
def test_get_no_allocation(client, authenticated_user, mocker, monkeypatch, mock_tapis_client):
    ...
    monkeypatch.setattr(mock_error, "response", MagicMock(json=MagicMock(return_value={}), status_code=500))
    mock_tapis_get.side_effect = mock_error

    mock_get_allocations = mocker.patch("portal.apps.datafiles.views.get_allocations")
    mock_get_allocations.return_value = {"hosts": {}}
...
def test_get_requires_push_keys(client, authenticated_user, mocker, monkeypatch, mock_tapis_client):
    ...
    mock_get_allocations = mocker.patch("portal.apps.datafiles.views.get_allocations")
    mock_get_allocations.return_value = {"hosts": {"frontera.tacc.utexas.edu": []}}

    system = {"host": "frontera.tacc.utexas.edu", "defaultAuthnMethod": "PKI_KEYS", "effectiveUserId": authenticated_user.username, "notes": {}}
...
def test_get_link(client, authenticated_user):
    Link.objects.create(tapis_uri="system/path", postit_url="https://postit")
...
def test_link_post(postits_create, authenticated_user, client):
    ...
    postits_create.assert_called_with(systemId="system", path="path", allowedUses=-1, validSeconds=31536000)
...
def test_link_put(postits_create, authenticated_user, mock_tapis_client, client):
    mock_tapis_client.files.deletePostIt.return_value = "OK"
    Link.objects.create(tapis_uri="system/path", postit_url="https://tenant/olduuid")
...
def test_tapis_file_view_get_is_logged_for_metrics(mock_indexer, client, authenticated_user, mock_tapis_client, tapis_file_listing_mock, logging_metric_mock):
...
@patch("portal.libs.agave.operations.tapis_indexer")
def test_tapis_file_view_put_is_logged_for_metrics(mock_indexer, client, authenticated_user, mock_tapis_client, tapis_file_listing_mock, logging_metric_mock):
    ...
    response = client.put("/api/datafiles/tapis/move/private/frontera.home.username/test.txt/", content_type="application/json", data=body)
    assert response.status_code == 200
...
def test_tapis_file_view_post_is_logged_for_metrics(
    mock_httpx, mock_indexer, client, authenticated_user, mock_tapis_client, logging_metric_mock, tapis_file_mock, requests_mock, text_file_fixture
):
...
def test_tapis_file_view_post_is_logged_for_metrics_exception(
    mock_post_handler, mock_indexer, client, authenticated_user, mock_tapis_client, logging_metric_mock, tapis_file_mock, requests_mock, text_file_fixture
):
...
def test_tapis_file_view_preview_supported_non_text_files(client, authenticated_user, mock_tapis_client, agave_file_listing_mock, EXTENSION, TYPE):
...
def test_tapis_file_view_preview_text_file(client, authenticated_user, mock_tapis_client, agave_file_listing_mock, requests_mock):
    mock_tapis_client.files.listFiles.return_value = [TapisResult(**f) for f in agave_file_listing_mock]
    mock_tapis_client.files.createPostIt.return_value = TapisResult(redeemUrl=POSTIT_HREF, expiration=None)
    requests_mock.get(POSTIT_HREF, text="file content")
    response = client.put(
        "/api/datafiles/tapis/preview/private/frontera.home.username/test_text.txt/",
        content_type="application/json",
        data={"href": "https//tapis.example/href"},
    )
    assert response.status_code == 200
...
def test_tapis_file_view_preview_other_text_file(client, authenticated_user, mock_tapis_client, agave_file_listing_mock, requests_mock):
    mock_tapis_client.files.listFiles.return_value = [TapisResult(**f) for f in agave_file_listing_mock]
    mock_tapis_client.files.createPostIt.return_value = TapisResult(redeemUrl=POSTIT_HREF, expiration=None)
    requests_mock.get(POSTIT_HREF, text="file content")
    response = client.put(
        "/api/datafiles/tapis/preview/private/frontera.home.username/some_other_txt_file_like_log.applog/",
        content_type="application/json",
        data={"href": "https//tapis.example/href"},
    )
    assert response.status_code == 200
...
def test_tapis_file_view_preview_unsupported_file(client, authenticated_user, mock_tapis_client, agave_file_listing_mock, requests_mock):
    mock_tapis_client.files.listFiles.return_value = [TapisResult(**f) for f in agave_file_listing_mock]
    mock_tapis_client.files.createPostIt.return_value = TapisResult(redeemUrl=POSTIT_HREF, expiration=None)
    requests_mock.get(POSTIT_HREF, text="file content")
    response = client.put(
        "/api/datafiles/tapis/preview/private/frontera.home.username/test.html/", content_type="application/json", data={"href": "https//tapis.example/href"}
    )
    assert response.status_code == 200
...
def test_tapis_file_view_preview_large_file(client, authenticated_user, mock_tapis_client, agave_file_listing_mock, requests_mock):
    agave_file_listing_mock[0]["size"] = 5000000
    mock_tapis_client.files.listFiles.return_value = [TapisResult(**f) for f in agave_file_listing_mock]
    mock_tapis_client.files.createPostIt.return_value = TapisResult(redeemUrl=POSTIT_HREF, expiration=None)
    requests_mock.get(POSTIT_HREF, text="file content")
    response = client.put(
        "/api/datafiles/tapis/preview/private/frontera.home.username/test.log/", content_type="application/json", data={"href": "https//tapis.example/href"}
    )
    assert response.status_code == 200
