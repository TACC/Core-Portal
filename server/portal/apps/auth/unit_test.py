...
class TestTapisOAuthBackend(TransactionTestCase):
    def setUp(self):
        ...
        self.mock_user_data_patcher = patch(
            "portal.apps.auth.backends.get_user_data", return_value={"username": "testuser", "firstName": "test", "lastName": "user", "email": "new@email.com"}
        )
    ...
    def test_new_user(self):
        ...
        self.mock_response.json.return_value = {"status": "success", "result": {"username": "testuser"}}
        result = self.backend.authenticate(backend="tapis", token="1234")
    ...
    def test_update_existing_user(self):
        ...
        user = get_user_model().objects.create_user(username="testuser", first_name="test", last_name="user", email="old@email.com")
        self.mock_response.json.return_value = {
            "status": "success",
            "result": {
                "username": "testuser",
