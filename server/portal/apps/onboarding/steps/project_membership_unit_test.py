...
def project_membership_step(settings, regular_user, tas_client, mock_rt):
    settings.PORTAL_USER_ACCOUNT_SETUP_STEPS = [
        {"step": "portal.apps.onboarding.steps.project_membership.ProjectMembershipStep", "settings": {"project_sql_id": 12345}}
    ]
...
def test_process_userlink(monkeypatch, project_membership_step_with_userlink, project_membership_log):
    ...
    project_membership_log.assert_called_with("Please confirm your request to use this portal.", data={"userlink": {"url": "/", "text": "Request Access"}})
