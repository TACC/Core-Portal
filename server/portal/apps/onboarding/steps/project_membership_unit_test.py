...
def test_process_user_is_member(
    monkeypatch, project_membership_step, project_membership_complete
):
    def mock_is_project_member():
        return True

    monkeypatch.setattr(
        project_membership_step, "is_project_member", mock_is_project_member
    )
...
def test_process_user_is_not_member(
    monkeypatch, project_membership_step, project_membership_log
):
    def mock_is_project_member():
        return False
    ...
    project_membership_log.assert_called_with(
        "Please confirm your request to use this portal.", data=None
    )


def test_process_userlink(
    monkeypatch, project_membership_step_with_userlink, project_membership_log
):
    def mock_is_project_member():
        return False

    monkeypatch.setattr(
        project_membership_step_with_userlink,
        "is_project_member",
        mock_is_project_member,
    )
    project_membership_step_with_userlink.process()
    project_membership_log.assert_called_with(
        "Please confirm your request to use this portal.",
        data={"userlink": {"url": "/", "text": "Request Access"}},
    )


def test_send_project_request(
    rf, project_membership_step, project_membership_log, mock_rt, regular_user
):
...
def test_client_action(
    regular_user, rf, monkeypatch, project_membership_step, project_membership_complete
):
...
def test_client_action_fail(
    rf, regular_user, monkeypatch, project_membership_step, project_membership_fail
):
