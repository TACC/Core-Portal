...
class ProjectMembershipStep(AbstractStep):
    ...
    def get_tracker(self):
        return Rt(
            settings.RT_HOST, settings.RT_UN, settings.RT_PW, http_auth=HTTPBasicAuth(settings.RT_UN, settings.RT_PW)
        )
            ...
            if tracker.login():
                result = tracker.create_ticket(
                    ...
                    Subject="{project} Project Membership Request for {username}".format(
                        project=self.default_project["title"], username=self.user.username
                    ),
                    Text=ticket_text,
            ...
            if "is already a member" in reason:
                self.complete(
                    "{username} is already a member of the {project}".format(
                        username=self.user.username, project=self.default_project["title"]
                    )
                )
            else:
                self.fail(
