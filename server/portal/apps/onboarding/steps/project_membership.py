...
class ProjectMembershipStep(AbstractStep):
    def __init__(self, user):
        ...
        """
        super().__init__(user)
        self.project_sql_ids = self.settings["project_sql_id"] if isinstance(self.settings["project_sql_id"], list) else [self.settings["project_sql_id"]]
        self.default_project_sql_id = self.settings.get("default_project_sql_id", self.project_sql_ids[0])
        self.default_project = self.get_tas_project(self.default_project_sql_id)
        self.user_confirm = "Request Project Access"
        self.staff_approve = "Add to {project}".format(project=self.default_project["title"])
    ...
    def get_tracker(self):
        return Rt(settings.RT_HOST, settings.RT_UN, settings.RT_PW, http_auth=HTTPBasicAuth(settings.RT_UN, settings.RT_PW))
        ...
        try:
            if tracker.login():
                result = tracker.create_ticket(
                    ...
                    Subject="{project} Project Membership Request for {username}".format(project=self.default_project["title"], username=self.user.username),
                    Text=ticket_text,
                ...
                self.log("Thank you for your request. It will be reviewed by TACC staff.", data={"ticket": result})
            else:
            ...
            self.fail("We were unable to submit a portal access request ticket on your behalf.")
    ...
    def add_to_project(self):
        ...
        try:
            ...
            if "is already a member" in reason:
                self.complete("{username} is already a member of the {project}".format(username=self.user.username, project=self.default_project["title"]))
            else:
                self.fail(
                    "{username} could not be added to {project} due to error {reason}".format(
                        project=self.default_project["title"], username=self.user.username, reason=reason
                    )
                )
                raise e
        ...
        if tracker.login():
            ...
            tracker.comment(
                ...
                text="User was not added to the {project} TAS Project (GID {gid}) at {base_url}".format(
                    project=self.default_project["title"], gid=self.default_project["gid"], base_url=settings.VANITY_BASE_URL
                ),
            )
    ...
    def close_project_request(self, deny=False):
        ...
        """.format(project=self.default_project["title"], base_url=settings.VANITY_BASE_URL)
        if tracker.login():
            tracker.reply(ticket_id, text=request_text)
            tracker.comment(
                ticket_id,
