...
def test_retry_step(client, settings, authenticated_user, mock_retry_step, mocker):
    ...
    response = client.get(f"/api/onboarding/user/{authenticated_user.username}", follow=True)
    mock_execute_single_step.apply_async.assert_called_with(args=[authenticated_user.username, "portal.apps.onboarding.steps.test_steps.MockStep"], countdown=2)
    result = json.loads(response.content)
    assert result["username"] == authenticated_user.username
    assert "steps" in result
    assert result["steps"][0]["step"] == "portal.apps.onboarding.steps.test_steps.MockStep"
...
def test_incomplete_post(client, authenticated_user):
    ...
    response = client.post(f"/api/onboarding/user/{authenticated_user}/", content_type="application/json", data=json.dumps({"action": "user_confirm"}))
    assert response.status_code == 400

    response = client.post(f"/api/onboarding/user/{authenticated_user}/", content_type="application/json", data=json.dumps({"step": "setupstep"}))
    assert response.status_code == 400
...
def test_get_no_profile(client, authenticated_staff, regular_user):
    ...
    assert not any([True for user in response_data["users"] if user["username"] == regular_user.username])
