...
class DjangoRt:
    def __init__(self):
        ...
        self.tracker = rt.Rt(
            ...
            self.rtUn,
            self.rtPw,
            http_auth=HTTPBasicAuth(self.rtUn, self.rtPw),
        )
        self.tracker.login()
    ...
    def getUserTickets(self, userEmail, status="ALL"):
        if not status == "ALL":
            ticket_list = self.tracker.search(
                Queue=rt.ALL_QUEUES,
                Requestors__exact=userEmail,
                Status__exact=status,
                order="-LastUpdated",
            )
        else:
            ticket_list = self.tracker.search(
                Queue=rt.ALL_QUEUES, Requestors__exact=userEmail, order="-LastUpdated"
            )
    ...
    def hasAccess(self, ticket_id, user=None):
        if user and ticket_id:
            ticket = self.tracker.get_ticket(ticket_id)
            if DjangoRt.contains_user(
                ticket.get("Requestors", ""), user
            ) or DjangoRt.contains_user(ticket.get("Cc", ""), user):
                return True
