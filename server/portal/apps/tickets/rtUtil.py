...
class DjangoRt:
    ...
    def getUserTickets(self, userEmail, status="ALL"):
        if not status == "ALL":
            ticket_list = self.tracker.search(
                Queue=rt.ALL_QUEUES, Requestors__exact=userEmail, Status__exact=status, order="-LastUpdated"
            )
        else:
    ...
    def hasAccess(self, ticket_id, user=None):
        if user and ticket_id:
            ticket = self.tracker.get_ticket(ticket_id)
            if DjangoRt.contains_user(ticket.get("Requestors", ""), user) or DjangoRt.contains_user(
                ticket.get("Cc", ""), user
            ):
                return True
