...
def mock_get_matching_history_entry(mocker, rt_ticket_history):
    ...
    mocker.patch(
        "portal.apps.tickets.api.views.TicketsHistoryView._get_matching_history_entry", return_value=last_entry
    )
...
def test_tickets_create_missing_required_description_or_subject(client, authenticated_user, mock_rtutil):
    # missing subject
    response = client.post("/api/tickets/", data={"problem_description": "problem_description"})
...
def test_tickets_create_missing_required_email(client, regular_user, mock_rtutil):
    ...
    response = client.post("/api/tickets/", data={"problem_description": "problem_description", "subject": "subject"})
    assert response.status_code == 400
...
def test_tickets_post_history_reply_with_text_and_attachment(
    ...
    attachment = (io.BytesIO(b"abcdef"), "test.jpg")
    response = client.post(
        "/api/tickets/1/history", data={"reply": "reply text", "attachments": attachment}, format="multipart"
    )
    assert response.status_code == 200
