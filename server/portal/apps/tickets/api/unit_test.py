...
def rt_ticket_history(scope="module"):
    yield json.load(
        open(os.path.join(settings.BASE_DIR, "fixtures/rt/ticket_history.json"))
    )
...
def mock_invalid_recaptcha(requests_mock):
    recaptchaSuccess = {
        "success": False,
        "challenge_ts": "2021-11-23T17:58:27Z",
        "hostname": "testkey.google.com",
    }
    requests_mock.post(
        "https://www.google.com/recaptcha/api/siteverify", json=recaptchaSuccess
    )
...
def mock_get_matching_history_entry(mocker, rt_ticket_history):
    ...
    mocker.patch(
        "portal.apps.tickets.api.views.TicketsHistoryView._get_matching_history_entry",
...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_tickets_create_missing_required_description_or_subject(
    client, authenticated_user, mock_rtutil
):
    # missing subject
    response = client.post(
        "/api/tickets/", data={"problem_description": "problem_description"}
    )
    assert response.status_code == 400

    # missing problem_description
    response = client.post("/api/tickets/", data={"subject": "subject"})
    assert response.status_code == 400
...
def test_tickets_create_missing_required_email(client, regular_user, mock_rtutil):
    ...
    response = client.post(
        "/api/tickets/",
        data={"problem_description": "problem_description", "subject": "subject"},
    )
    assert response.status_code == 400
...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_tickets_create_unauthencated_invalid_recaptcha(
    client, regular_user, mock_rtutil, mock_invalid_recaptcha
):
    response = client.post(
        "/api/tickets/",
        data={
...
def test_tickets_get_history_handle_service_accounts(
    service_account, client, authenticated_user, mock_rtutil, rt_ticket_history
):
...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_tickets_get_history_no_access(
    client, authenticated_user, mock_rtutil_no_access
):
    response = client.get("/api/tickets/1/history")
...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_tickets_post_history_reply_with_text(
    client, authenticated_user, mock_rtutil, mock_get_matching_history_entry
):
    response = client.post("/api/tickets/1/history", data={"reply": "reply text"})
    assert response.status_code == 200
...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_tickets_post_history_reply_with_multiline_text(
    client, authenticated_user, mock_rtutil, mock_get_matching_history_entry
):
    response = client.post("/api/tickets/1/history", data={"reply": "reply text"})
    assert response.status_code == 200
...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_tickets_post_history_reply_missing_text(
    client, authenticated_user, mock_rtutil
):
...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_tickets_post_history_reply_no_access(
    client, authenticated_user, mock_rtutil_no_access
):
    response = client.post("/api/tickets/1/history")
    assert response.status_code == 403
...
@pytest.mark.django_db(transaction=True, reset_sequences=True)
def test_tickets_post_history_reply_with_text_and_attachment(
    client, authenticated_user, mock_rtutil, mock_get_matching_history_entry
):
    attachment = (io.BytesIO(b"abcdef"), "test.jpg")
    response = client.post(
        "/api/tickets/1/history",
        data={"reply": "reply text", "attachments": attachment},
        format="multipart",
    )
    assert response.status_code == 200
