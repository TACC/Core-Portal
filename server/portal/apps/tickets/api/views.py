...
class TicketsView(BaseApiView):
    ...
    def post(self, request):
        ...
        return utils.create_ticket(
            username, first_name, last_name, email, cc, subject, problem_description, attachments, info, meta
        )
    ...
    def _get_ticket_history(self, rt, requesting_username, ticket_id):
        ...
        for entry in ticket_history:
            ...
            if entry["Creator"] in SERVICE_ACCOUNTS:
                ...
                submitted_for_user = (
                    re.search(r"\[Reply submitted on behalf of (.*?)\]", entry["Content"].splitlines()[-1])
                    if entry["Content"]
                    else False
                )
                ...
                if not submitted_for_user and entry["Type"] == "Create":
                    submitted_for_user = (
                        re.findall(r"authenticated_user:[\r\n]+([^\r\n]+)", entry["Content"], re.MULTILINE)
                        if entry["Content"]
                        else False
                    )
                    if submitted_for_user:
