...
class TicketsView(BaseApiView):
    ...
    def post(self, request):
        ...
        cc = data.get("cc", [])
        attachments = [
            (f.name, ContentFile(f.read()), f.content_type)
...
class TicketsHistoryView(BaseApiView):
    ...
    def _get_ticket_history(self, rt, requesting_username, ticket_id):
        ...
        ticket_history = list(
            filter(lambda h: h["Type"] in ALLOWED_HISTORY_TYPES, ticket_history)
        )
        for entry in ticket_history:
            ...
            if entry["Creator"] in SERVICE_ACCOUNTS:
                ...
                submitted_for_user = (
                    re.search(
                        r"\[Reply submitted on behalf of (.*?)\]",
                        entry["Content"].splitlines()[-1],
                    )
                    if entry["Content"]
                    else False
                )
            ...
            if entry["Type"] == "Create":
                entry["Content"] = entry["Content"][
                    : entry["Content"].rfind(METADATA_HEADER)
                ]

            entry["Creator"] = (
                "RT System" if entry["Creator"] == "RT_System" else entry["Creator"]
            )

            entry["IsCreator"] = (
                True if requesting_username == entry["Creator"] else False
            )

            known_user = (
                get_user_model().objects.filter(username=entry["Creator"]).first()
            )
            if known_user:
                entry["Creator"] = "{} {}".format(
                    known_user.first_name, known_user.last_name
                )
    ...
    def post(self, request, ticket_id):
        ...
        rt = rtUtil.DjangoRt()
        result = rt.replyToTicket(
            ticket_id=ticket_id, reply_text=modified_reply, files=attachments
        )
        if not result:
