...
class TestUserApiViews(TestCase):
    ...
    def setUp(self):
        ...
        token = TapisOAuthToken(access_token="1234fsf", refresh_token="123123123", expires_in=14400, created=1523633447)
        token.user = user
        token.save()
        user.is_staff = False
...
class TestGetAllocations(TestCase):
    ...
    def test_allocations_returned(self):
        ...
        active_expected = [
            {
                ...
                "projectId": "Test-ID",
                "systems": [
                    {"allocation": {"resource": "Frontera", "status": "Active"}, "host": "frontera.tacc.utexas.edu", "name": "Frontera", "type": "HPC"}
                ],
                "title": "Big Project",
                "pi": "Test User",
            },
            {
                ...
                "projectId": "Test-ID",
                "systems": [{"allocation": {"resource": "Rodeo2", "status": "Active"}, "host": "rodeo.tacc.utexas.edu", "name": "Rodeo", "type": "STORAGE"}],
                "title": "A Proj",
                "pi": "Another User",
            },
        ]
        ...
        inactive_expected = [
            {
                ...
                "projectId": "Test-ID",
                "systems": [
                    {"allocation": {"resource": "Stampede4", "status": "Inactive"}, "host": "stampede2.tacc.utexas.edu", "name": "Stampede2", "type": "HPC"}
                ],
                "title": "Old Proj",
                "pi": "Old User",
            }
        ...
        hosts_expected = {"rodeo.tacc.utexas.edu": ["Proj-Code"], "frontera.tacc.utexas.edu": ["Big-Proj"]}

        data_expected = {"active": active_expected, "inactive": inactive_expected, "hosts": hosts_expected, "portal_alloc": "test"}
...
def test_search_tas(client, authenticated_user, mock_tas_zeep_client, mock_tas_account1, mock_tas_account2):
    ...
    assert response.json() == {
        "result": [
            {"username": "username1", "email": "user1@user.com", "firstName": "firstName1", "lastName": "commonLastName"},
            {"username": "username2", "email": "user2@user.com", "firstName": "firstName2", "lastName": "commonLastName"},
        ]
    }
