...
class AllocationsView(BaseApiView):
    ...
    def get(self, request):
        ...
        filtered_projects = [
            project
            for project in data["active"]
            if project.get("projectName") not in settings.ALLOCATIONS_TO_EXCLUDE
        ]
...
class TasUsersView(BaseApiView):
    ...
    def _getSOAPTASClient(self):
        ...
        try:
            client = Client(
                "https://tas.tacc.utexas.edu/TASWebService/PortalService.asmx?WSDL",
    ...
    def put(self, request):
        ...
        if not is_pi:
            return JsonResponse(
                {
                    "message": "Forbidden: Project roles can only be assigned by the Project PI."
                },
                status=403,
            )
