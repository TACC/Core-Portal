...
def get_project_users_from_name(project_name):
    ...
    r = requests.get(
        "{0}/v1/projects/name/{1}/users".format(settings.TAS_URL, project_name),
        auth=auth,
    )
    resp = r.json()
...
def get_project_users_from_id(project_id):
    ...
    r = requests.get(
        "{0}/v1/projects/{1}/users".format(settings.TAS_URL, project_id), auth=auth
    )
    resp = r.json()
    if resp["status"] == "success":
        return resp["result"]
...
def get_project_from_name(project_name):
    ...
    r = requests.get(
        "{0}/v1/projects/name/{1}".format(settings.TAS_URL, project_name), auth=auth
    )
    resp = r.json()
    if resp["status"] == "success":
        return resp["result"]
...
def get_project_from_id(project_id):
    ...
    r = requests.get(
        "{0}/v1/projects/{1}".format(settings.TAS_URL, project_id), auth=auth
    )
    resp = r.json()
    if resp["status"] == "success":
        return resp["result"]
...
def get_per_user_allocation_usage(allocation_id):
    ...
    r = requests.get(
        "{0}/v1/allocations/{1}/usage".format(settings.TAS_URL, allocation_id),
        auth=auth,
    )
    resp = r.json()
    if resp["status"] == "success":
...
def remove_user(project_id, user_id):
    ...
    r = requests.delete(
        "{0}/v1/projects/{1}/users/{2}".format(settings.TAS_URL, project_id, user_id),
        auth=auth,
    )
    resp = r.json()
    if resp["status"] != "success":
...
def check_user_groups(username, groups):
    try:
        return any(
            ...
            for group in groups
            for user in get_project_users_from_name(group)
        )
    except Exception as e:
        logger.error(
            "Issue checking user groups for user:{} which failed with the following exception:{}".format(
                username, e
            )
        )
        return False
