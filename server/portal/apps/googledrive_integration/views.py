...
def initialize_token(request):
    redirect_uri = "https://{}{}".format(
        request.get_host(), reverse("googledrive_integration:oauth2_callback")
    )
    flow = google_auth_oauthlib.flow.Flow.from_client_config(
        get_client_config(), scopes=["https://www.googleapis.com/auth/drive"]
    )
    ...
    request.session["googledrive"] = {"state": state}
...
def oauth2_callback(request):
    ...
    try:
        ...
        authorization_response = "https://{}{}".format(
        ...
        if not credentials.refresh_token:
            ...
            logger.error(
                "GoogleDriveUserToken refresh_token cannot be null, revoking previous access and restart flow."
            )
            requests.post(
        ...
        GoogleDriveUserToken.objects.update_or_create(
            user=request.user, defaults={"credentials": credentials}
        )
...
def disconnect(request):
    ...
    try:
        ...
        if status_code == 200:
            ...
            logger.error(
                "Disconnect Google Drive; google drive account revoke error.",
        ...
        logger.warn(
            "Disconnect Google Drive; GoogleDriveUserToken does not exist.",
            extra={"user": request.user},
        )

    except Exception as e:
        logger.error(
            "Disconnect Google Drive; GoogleDriveUserToken delete error.",
            extra={"user": request.user},
        )
