...
def mock_flow(mocker):
    mock_flow = mocker.patch(
        "portal.apps.googledrive_integration.views.google_auth_oauthlib.flow"
    )
    mock_flow.Flow.from_client_config.return_value.authorization_url.return_value = (
        "test_auth_url",
        "test_state",
    )
    mock_flow.Flow.from_client_config.return_value.credentials = Credentials(
...
def test_initialize(django_user_model, client, mock_flow):
    ...
    mock_flow.Flow.from_client_config.assert_called_with(
        {
            "web": {
                "client_id": "test",
    ...
    mock_flow.Flow.from_client_config.return_value.authorization_url.assert_called_with(
        access_type="offline"
    )

    assert (
        mock_flow.Flow.from_client_config.return_value.redirect_uri
        == "https://testserver/accounts/applications/googledrive/oauth2/"
    )
...
def test_redirect(django_user_model, client, mock_flow):
    ...
    session.save()
    response = client.get(
        "/accounts/applications/googledrive/oauth2/", {"state": "12345"}
    )
...
def test_disconnect(django_user_model, client, mock_flow, mock_request):
    ...
    GoogleDriveUserToken.objects.create(user=user, credentials=credentials)
    assert user.googledrive_user_token.credentials.token == "asdf"
