...
class CustomMessageTemplate(models.Model):
    ...
    MESSAGE_TYPES = [("info", "Info"), ("success", "Success"), ("warning", "Warn"), ("error", "Error")]
    ...
    def to_dict(self):
        return {"id": self.id, "component": self.component, "message_type": self.message_type, "dismissible": self.dismissible, "message": self.message}

    def __str__(self):
        return "%s | %s | %s | %s" % (self.message_type, self.component, ("dismissible" if self.dismissible else "not dismissible"), self.message[0:20])
