...
class IntroMessagesView(BaseApiView):
    def get(self, request, *args, **kwargs):
        messages_array = IntroMessages.objects.filter(user=request.user).values(
            "component", "unread"
        )
    ...
    def put(self, request, *args):
        ...
        for component_name, component_value in body.items():
            IntroMessages.objects.update_or_create(
                user=request.user,
                component=component_name,
                defaults={"unread": component_value},
            )
...
class CustomMessagesView(BaseApiView):
    def get(self, request, *args, **kwargs):
        ...
        messages = [
            get_or_create_custom_messages(request.user, template)
    ...
    def put(self, request, *args):
        ...
        message = CustomMessages.objects.get(
            user=request.user, template__id=template_id
        )
