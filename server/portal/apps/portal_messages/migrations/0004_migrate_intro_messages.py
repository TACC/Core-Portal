...
def migrate_intro_messages(apps, schema_editor):
    ...
    for component, message in intro_messages.items():
        template = CustomMessageTemplate.objects.create(component=component, message_type="info", dismissible=True, message=message)

        for intro_message in IntroMessages.objects.filter(component=component):
            CustomMessages.objects.create(user=intro_message.user, template=template, unread=intro_message.unread)
