    ...
    def get(self, request, *args, **kwargs):
        ...
        if event_types:
            notifs = Notification.objects.filter(
                event_type__in=event_types, deleted=False, user=request.user.username, **query_params
            ).order_by("-datetime")
            total = Notification.objects.filter(
                event_type__in=event_types, deleted=False, user=request.user.username
            ).count()
            unread = Notification.objects.filter(
                event_type__in=event_types, deleted=False, read=False, user=request.user.username
            ).count()
        else:
            notifs = Notification.objects.filter(deleted=False, user=request.user.username, **query_params).order_by(
                "-datetime"
            )
    ...
    def patch(self, request, *args, **kwargs):
        ...
        if nid == "all" and read is True:
            if event_types is not None:
                notifs = Notification.objects.filter(
                    deleted=False, read=False, event_type__in=event_types, user=request.user.username
                )
            else:
