...
def sync_files_without_metadata(self, user_access_token, project_id: str):
    ...
    tapis_files_listing = client.files.listFiles(
        systemId=project_id, path="/", recurse=True
    )
    ...
    entity_cache = {
        entity.uuid: entity
        for entity in ProjectMetadata.objects.filter(uuid__in=required_uuids)
    }
    ...
    for file in files:
        ...
        if not entity:
            logger.warning(
                f"Parent entity {parent_uuid} for file {file_path} does not exist"
            )
            continue
        ...
        if file_path not in file_paths_set:
            new_file_obj = create_file_obj(
                project_id, file.name, file.size, file_path, {"data_type": "file"}
            )
            files_to_add_dict[entity.uuid] = files_to_add_dict.get(entity.uuid, []) + [
                new_file_obj
            ]

    for entity_uuid, file_objs in files_to_add_dict.items():
        logger.info(
            f"Adding {len(file_objs)} files to entity {entity_uuid} in project {project_id}"
        )
...
def process_file(
    ...
    if file and file_obj:
        ...
        try:
            ...
            logger.error(
                f"Could not generate advanced image for {file_name} due to error: {e}"
            )
        ...
        try:
            ...
            client.files.insert(
                systemId=project_id, path=thumbnail_path, file=thumbnail
            )
        except Exception as e:
        ...
        try:
            ...
            client.files.insert(
                systemId=project_id, path=histogram_img_path, file=histogram_img
            )
            client.files.insert(
                systemId=project_id, path=histogram_csv_path, file=histogram_csv
            )
        except Exception as e:
        ...
        try:
            ...
            client.files.insert(
                systemId=project_id, path=animation_path, file=animation
            )
        except Exception as e:
