...
class ProjectsManager(object):
    ...
    def create(self, title):
        ...
        project_id = "{prefix}-{prjId}".format(
            prefix=settings.PORTAL_PROJECTS_ID_PREFIX, prjId=prjId
        )

        try:
            prj = Project.create(
                self.user.tapis_oauth.client, title, project_id, self.user
            )
        except ValueError:
            ...
            logger.info(
                "Updating ProjectId to latest project dir or storage system id: {}".format(
                    max_value_found
                )
            )
        ...
        METRICS.info(
            "user:{} created project: id={}, title:{}".format(
                self.user.username, project_id, title
            )
        )
    ...
    def list(self, offset=0, limit=100):
        ...
        return [
            prj.storage
            for prj in Project.listing(
                self.user.tapis_oauth.client, offset=offset, limit=limit
            )
        ]
    ...
    def search(self, query_string, offset=0, limit=100):
        ...
        search_result = search_result.query(
            "query_string", query=query_string, minimum_should_match="80%"
        )
