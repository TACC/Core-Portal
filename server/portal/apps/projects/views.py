...
class ProjectsApiView(BaseApiView):
    ...
    def get(self, request, root_system=None):
        ...
        if query_string:
            ...
            wildcard_query = Q("wildcard", title=f"*{query_string.lower()}*") | Q(
                "wildcard", id=f"*{query_string.lower()}*"
            )
            ...
            for project in listing:
                try:
                    project_meta = ProjectMetadata.objects.get(
                        models.Q(value__projectId=project["id"])
                    )
                    project.update(
                        get_ordered_value(project_meta.name, project_meta.value)
                    )
                    project["projectId"] = project["id"].split(
                        f"{settings.PORTAL_PROJECTS_SYSTEM_PREFIX}."
                    )[1]
                except Exception as e:
                    LOGGER.exception(
                        f"Failed to retrieve metadata for project {project['id']}: {e}"
                    )
    ...
    def post(self, request):  # pylint: disable=no-self-use
        ...
        if settings.PORTAL_PROJECTS_ENABLE_METADATA:
            ...
            if cover_image:
                project_metadata["cover_image"] = (
                    f"media/{settings.PORTAL_PROJECTS_ID_PREFIX}-{workspace_number}/cover_image/{cover_image.name}"
                )
        ...
        system_id = create_shared_workspace(
            client,
            title,
            description,
            keywords,
        ...
        return JsonResponse({"status": 200, "response": {"id": system_id}})
...
class ProjectInstanceApiView(BaseApiView):
    ...
    def get(self, request, project_id=None, system_id=None):
        ...
        if system_id is not None:
            project_id = system_id.split(f"{settings.PORTAL_PROJECTS_SYSTEM_PREFIX}.")[
                1
            ]

        if (
            system_id
            and settings.PORTAL_PROJECTS_PUBLISHED_SYSTEM_PREFIX
            and system_id.startswith(settings.PORTAL_PROJECTS_PUBLISHED_SYSTEM_PREFIX)
        ):
        ...
        if settings.PORTAL_PROJECTS_ENABLE_METADATA:
            try:
                project = ProjectMetadata.objects.get(
                    models.Q(
                        value__projectId=f"{settings.PORTAL_PROJECTS_SYSTEM_PREFIX}.{project_id}"
                    )
                )
                ...
                if prj.get("cover_image") is not None:
                    ...
                    postit = service_client.files.createPostIt(
                        systemId=root_system,
                        path=prj["cover_image"],
                        allowedUses=-1,
                        validSeconds=86400,
                    )
                    prj["file_url"] = postit.redeemUrl

                if not prj.get("is_review_project", False) and not prj.get(
                    "is_published_project", False
                ):
                    sync_files_without_metadata.delay(
                        client.access_token.access_token,
                        f"{settings.PORTAL_PROJECTS_SYSTEM_PREFIX}.{project_id}",
                    )
            except Exception as e:
                LOGGER.exception(
                    f"Failed to retrieve metadata for project {project_id}: {e}"
                )
    ...
    @transaction.atomic
    def patch(
        self, request, project_id=None, system_id=None
        ...
        if system_details.owner == request.user.username or check_project_admin_group(
            request.user
        ):
            workspace_def = update_project(
                client, project_id, title, description, keywords
            )
        else:
        ...
        if settings.PORTAL_PROJECTS_ENABLE_METADATA:
            ...
            if cover_image:
                project_metadata["cover_image"] = (
                    f"media/{project_id}/cover_image/{cover_image.name}"
                )
        ...
        return JsonResponse({"status": 200, "response": workspace_def})
...
class ProjectMembersApiView(BaseApiView):
    ...
    def patch(self, request, project_id):
        ...
        try:
            ...
            LOGGER.error("Invalid action.", extra=request.POST.dict(), exc_info=True)
            raise ApiException("Invalid action.", 403, request.POST.dict())
    ...
    def add_member(self, request, project_id, **data):
        ...
        return JsonResponse({"status": 200, "response": resp})
    ...
    def change_project_role(self, request, project_id, **data):
        ...
        prj = ProjectsManager(request.user).change_project_role(
            project_id, username, old_role, new_role
        )
    ...
    def change_system_role(self, request, project_Id, **data):
        ...
        role_map = {"GUEST": "reader", "USER": "writer"}
...
class ProjectEntityView(BaseApiView):
    ...
    def patch(self, request: HttpRequest, project_id: str):
        ...
        if value["data_type"] == "file":
            ...
            try:
                new_name = move_entity(
                    client, project_id, path, updated_path, value, entity_uuid
                )
                patch_entity_and_node(
                    project_id, value, path, updated_path, new_name, entity_uuid
                )
            except Exception as exc:
                raise ApiException(
                    "Error updating entity metadata", status=500
                ) from exc
    ...
    def post(self, request: HttpRequest, project_id: str):
        ...
        add_node_to_project(
            project_id, parent_node["id"], new_meta.uuid, new_meta.name, value["name"]
        )
