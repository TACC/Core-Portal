...
def _transfer_files(client, source_system_id, dest_system_id):
    ...
    filtered_files = [
        ...
        if file.name != settings.TAPIS_DEFAULT_TRASH_NAME
    ]
...
def _check_transfer_status(service_client, transfer_task_id):
    transfer_details = service_client.files.getTransferTask(
        transferTaskId=transfer_task_id
    )
...
def publish_project_callback(
    review_project_id, published_project_id, archive_project_id
):
    service_client = service_account()
    update_and_cleanup_review_project(
        review_project_id, PublicationRequest.Status.APPROVED
    )
...
def publication_request_callback(
    ...
    with transaction.atomic():
        ...
        for reviewer in publication_reviewers:
            ...
            logger.info(
                f"Added reviewer {reviewer} to review system {review_system_id}"
            )

        if not settings.DEBUG:
            send_publication_in_review_email_to_authors.apply_async(
                args=[source_system_id]
            )
            send_publication_submitted_for_review_email_to_reviewers.apply_async(
                args=[review_system_id]
            )
...
def upload_metadata_file(project_id: str, project_json: str):
    ...
    logger.debug(
        "Created metadata file for %s at tapis://%s/%s",
        project_id,
        published_root,
        upload_full_path,
    )
...
def archive_publication_files(project_id: str):
    ...
    published_root_dir = client.systems.getSystem(
        systemId=published_root_system
    ).rootDir
...
def publish_project(self, project_id: str, version: Optional[int] = 1):
    ...
    with transaction.atomic():
        ...
        publication_tree: nx.DiGraph = nx.node_link_graph(
            project_meta.project_graph.value
        )
        ...
        try:
            ...
            logger.info(
                f"Attempting to mint DataCite DOI for project {project_id}, existing DOI: {existing_doi}"
            )
            ...
            logger.info(
                f"Successfully minted DataCite DOI for project {project_id}: {doi}"
            )
        ...
        if not settings.DEBUG:
            try:
                ...
                logger.error(
                    f"Error publishing DataCite DOI for project {project_id}: {e}"
                )
...
def copy_graph_and_files_for_review_system(
    ...
    logger.info(
        f"Starting copy task for system {source_system_id} to system {review_system_id}"
    )
...
def poll_tapis_file_transfer(self, transfer_task_id, is_review, **kwargs):
    logger.info(
        f"Starting post transfer task for transfer id {transfer_task_id} with arguments: {kwargs}"
    )
    ...
    try:
        ...
        if transfer_status in ["PENDING", "IN_PROGRESS"]:
            ...
            self.apply_async(
                args=(transfer_task_id, is_review), kwargs=kwargs, countdown=30
            )
            return
        ...
        # Handle completed transfer
            ...
            logger.info(
                f"Transfer {transfer_task_id} completed successfully with arguments: {kwargs}"
            )
            ...
            logger.error(
                f"Error processing transfer {transfer_task_id}: Transfer status is {transfer_status}"
            )
            raise Exception(
                f"Transfer {transfer_task_id} failed with status {transfer_status}"
            )

    except Exception as e:
        logger.error(
            f"Error processing transfer {transfer_task_id} with arguments {kwargs}: {e}"
        )
        self.retry(exc=e, countdown=30)


@transaction.atomic
def update_and_cleanup_review_project(
    review_project_id: str, status: PublicationRequest.Status
):

    client = service_account()

    workspace_id = review_project_id.split(
    ...
    logger.info(
        f"Updated publication request for review project {review_project_id} to {status}."
    )
    ...
    for reviewer in reviewers:
        try:
            ...
            logger.info(
                f"Removed reviewer {reviewer.username} from review system {review_project_id}"
            )
        except Exception:
            logger.error(
                f"Error removing reviewer {reviewer.username} from review system {review_project_id}"
            )
            continue

    client.files.delete(systemId=review_project_id, path="/")
    client.systems.deleteSystem(systemId=review_project_id)
    review_project_graph = ProjectMetadata.objects.get(
        name=constants.PROJECT_GRAPH, base_project=review_project
    )
