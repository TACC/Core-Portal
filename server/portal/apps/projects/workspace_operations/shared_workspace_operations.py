...
def create_workspace_dir(workspace_id: str, system_id=settings.PORTAL_PROJECTS_ROOT_SYSTEM_NAME, **kwargs) -> str:
    ...
    path = f"{workspace_id}"
    client.files.mkdir(
        systemId=system_id, path=path, headers={"X-Tapis-Tracking-ID": kwargs.get("tapis_tracking_id", "")}
    )
    return path


def create_workspace_system(
    client, workspace_id: str, title: str, description: str, keywords: str, owner=None, system_id=None, root_dir=None
) -> str:
...
def create_shared_workspace(
    client: Tapis, title: str, description: str, keywords: str, owner: str, predefind_workspace_number=None, **kwargs
):
    """
...
def add_user_to_workspace(
    client: Tapis, workspace_id: str, username: str, role="writer", system_id=None, system_name=None
):
    """
...
def update_project(client, workspace_id: str, title: str, description: str, keywords: str):
    ...
    client.systems.patchSystem(
        systemId=system_id, notes={"title": title, "description": description, "keywords": keywords}
    )
...
def list_projects(client, root_system_id=None):
    ...
    community_system = next(
        (system for system in settings.PORTAL_DATAFILES_STORAGE_SYSTEMS if system["scheme"] == "community"), None
    )

    if community_system and not is_review_system and not is_publication_system:
        community_data_query = (
            f"(id.like.{settings.PORTAL_PROJECTS_SYSTEM_PREFIX}.*)~(rootDir.like.{community_system['homeDir']}*)"
        )
    else:
...
def create_publication_workspace(
    ...
    system_prefix = (
        settings.PORTAL_PROJECTS_REVIEW_SYSTEM_PREFIX if is_review else settings.PORTAL_PROJECTS_PUBLISHED_SYSTEM_PREFIX
    )
    root_system_name = (
