...
def test_project_create(
    mock_owner, portal_project, agave_client, mock_project_save_signal
):
...
@pytest.mark.skip(reason="TODOv3: deprecate with projects")
def test_project_create_dir_failure(
    mock_owner, portal_project, agave_client, mock_project_save_signal
):
...
@pytest.mark.skip(reason="TODOv3: deprecate with projects")
def test_project_create_storage_failure(
    mock_owner, portal_project, agave_client, mock_project_save_signal
):
...
@pytest.mark.skip(reason="TODOv3: deprecate with projects")
def test_metadata_create_on_project_load(
    agave_client, mock_owner, mock_project_save_signal
):
    agave_client.systems.listRoles.return_value = [
...
def test_project_change_system_role(agave_client, mock_owner, mock_project_save_signal):
    ...
    agave_client.systems.updateRole.assert_called_with(
        body={"role": "USER", "username": "username"}, systemId="cep.test.PRJ-123"
    )


@pytest.mark.skip(reason="TODOv3: deprecate with projects")
def test_project_change_project_role(
    agave_client, mock_owner, mock_project_save_signal, mocker
):
...
@pytest.mark.skip(reason="TODOv3: deprecate with projects")
def test_get_latest_project_storage(
    mock_owner,
    portal_project,
    agave_client,
