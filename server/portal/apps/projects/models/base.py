...
def set_storage_auth(storage):
    ...
    if not settings.PORTAL_PROJECTS_PRIVATE_KEY:
        ...
        pub_key = EncryptionUtil.export_key(
            EncryptionUtil.create_public_key(key), "OpenSSH"
        )
        ...
        try:
            ...
            logger.error(
                "There was an error saving the ssh keys locally: %s", exc, exc_info=True
            )
    else:
...
class Project(object):
    ...
    def _get_metadata(self):
        ...
        try:
            ...
            admins = list(
                filter(
                    lambda role_tuple: role_tuple[0] != "wma_prtl"
                    and (role_tuple[1] == "ADMIN" or role_tuple[1] == "OWNER"),
                    roles,
                )
            )
            if len(admins) == 1:
    ...
    def create(cls, client, title, project_id, owner):
        ...
        try:
            storage = cls._create_storage(
                title, ProjectsUtils.project_id_to_system_id(project_id), project_id
            )
