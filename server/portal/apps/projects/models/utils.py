...
def get_latest_project_storage(max_project_id=None):
    ...
    all_projects = []
    while True:
        prjs = [p for p in Project.listing(service_account(), offset=offset, limit=limit)]
        all_projects += prjs
        offset += limit
        if len(prjs) < limit:
            break
...
def get_latest_project_directory(max_project_id=None):
    ...
    """
    latest = -1
    for f in iterate_listing(service_account(), system=settings.PORTAL_PROJECTS_ROOT_SYSTEM_NAME, path="/"):
        name = f["name"]
