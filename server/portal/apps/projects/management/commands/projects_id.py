...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        update_group.add_argument(
            ...
            action="store_true",
            help="Update project id DB value using value derived from latest storage system project id or latest directory project id (whichever is higher).",
        )
        parser.add_argument("--max-project-id", action="store", type=int, help="Ignore project ids larger than a certain value")

    def handle(self, *args, **options):
        """Handle command."""
        max_project_id = options["max_project_id"]
        if max_project_id:
            self.stdout.write(f"NOTE(!!!!): Ignoring project ids >= {max_project_id} when processing/updating the storage systems and directories")
