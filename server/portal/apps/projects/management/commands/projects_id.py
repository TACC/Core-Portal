...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        update_group.add_argument(
            "--update", action="store", type=int, help="Update project id DB value."
        )
        update_group.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        if max_project_id:
            self.stdout.write(
                ...
                "processing/updating the storage systems and directories".format(
                    max_project_id
                )
            )
        ...
        self.stdout.write(
            "Latest storage system project id: {}".format(latest_storage_system_id)
        )
        ...
        try:
            ...
            self.stdout.write(
                "Latest project id in ProjectId model: {}".format(model_project_id)
            )
        ...
        if options.get("update"):
            self.stdout.write(
                "Updating to user provided value of: {}".format(options.get("update"))
            )
            ...
            self.stdout.write(
                "Updating to value latest storage system id: {}".format(max_value_found)
            )
