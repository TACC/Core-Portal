...
def disconnect_signal():
    yield signals.post_save.disconnect(
        sender=LegacyProjectMetadata, dispatch_uid="index_project"
    )
...
def mock_service_account(mocker):
    mock = mocker.patch(
        "portal.apps.projects.management.commands.migrate-projects.service_account"
    )
...
@pytest.mark.skip(reason="role management different in v3")
def test_migrate_projects_wrong_admins(
    regular_user, mock_project_metadata, mock_project_storage, mock_service_account
):
    mock_project_storage.return_value.roles.to_dict.return_value = {
        "wma_prtl": "OWNER",
        "username": "ADMIN",
