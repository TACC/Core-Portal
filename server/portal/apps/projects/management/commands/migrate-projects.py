        ...
        for meta in metadatas:
            if not meta.pi:
                try:
                    ...
                    if len(admins) != 1:
                        raise Exception(
                            "Not exactly one admin for {project_id}".format(
                    ...
                    logger.info(
                        "Set {admin} as PI on {project_id}".format(
                            admin=admin.username, project_id=meta.project_id
                        )
                    )

                except Exception as e:
                    logger.error(
                        "Could not migrate {project_id}".format(
                            project_id=meta.project_id
                        )
                    )
                    logger.exception(e)
