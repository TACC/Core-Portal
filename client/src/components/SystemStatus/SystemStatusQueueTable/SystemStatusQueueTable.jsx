...
import { LoadingSpinner, Section, SectionTableWrapper, SectionMessage } from '_common';
...
export const SystemStatusQueueTable = ({ system }) => {
  ...
  useEffect(async () => {
    ...
    const columns = useMemo(() => [
        ...
        }, 
        {
            ...
            Cell: ({ cell: { value } }) => {
                const { down } = value;
                return (
                    down ? <Pill type="warning">Closed</Pill> : <Pill type="success">Open</Pill>
                )
            }
        },
        {
    ...
    const { getTableProps, getTableBodyProps, rows, prepareRow, headerGroups } = useTable({ columns, data: systemQueues ?? [] })
