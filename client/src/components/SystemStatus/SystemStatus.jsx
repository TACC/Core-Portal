...
const Layout = ({ hostname }) => {
    ...
    const { loading, error: loadingError, list: systemList } = useSelector((state) => state.systemMonitor);
    ...
    if (loadingError) {
      return (
        <Section className={styles['no-results-message']}>
          <SectionMessage type="info">Unable to gather system information</SectionMessage>
      </Section>
      );
    }
    if (loading) {
            ...
            content = {
                <>
                    ...
                    <SectionTableWrapper
                        ...
                        contentShouldScroll
                    >
                        <SystemStatusQueueTable system={selectedSystem}></SystemStatusQueueTable>
                    </SectionTableWrapper>
                </>
            }
        />
...
const DefaultSystemRedirect = () => {
  ...
  useEffect(() => {
    ...
    history.push(
      `/workbench/system-status/${defaultSystem.hostname}`
    );
  }, [systemList]);
  return <></>;
};
