...
function PublishedDatasetEntityDetail({ params }) {
  ...
  const groupFilesByBaseName = (files) => {
    ...
    const groups = files.reduce((map, file) => {
      const suffix = processedFileSuffixes.find((suf) =>
        file.name.endsWith(suf)
      );
      const baseName = suffix ? file.name.slice(0, -suffix.length) : file.name;
  ...
  const getDigitalDatasetLink = (digitalDataset) => {
    ...
    if (urlRegex.test(digitalDataset)) {
      return (
        <a href={`${digitalDataset}`} target="_blank" rel="noreferrer">
          {digitalDataset}
        </a>
      );
                    ...
                    return (
                      <tr key={key}>
                        <th className="c-data-list__key">
                          Reference Digital Dataset
                        </th>
                        <td className="c-data-list__value">
                          {getDigitalDatasetLink(value)}
                        </td>
                      </tr>
                    );
                  }
          ...
          <section id="data" className="o-section">
            <ul className="c-card-list">
              {paginationData.currentFileGroups.map(({ raw, processed }) => {
                ...
                const gifFile = processed.find((file) =>
                  file.name.endsWith('.gif')
                );
                const histogramFile = processed.find((file) =>
                  file.name.endsWith('.histogram.jpg')
                );
                const histogramCsvFile = processed.find((file) =>
                  file.name.endsWith('.histogram.csv')
                );
                ...
                if (thumbnailFile) {
                  ...
                  raw.value?.isAdvancedImageFile !== true &&
                  processed.length === 0
                ) {
                    ...
                    <table className="c-data-list c-data-list--is-vert c-data-list--is-narrow c-data-list--should-truncate-values">
                      <tbody>
                        {Object.entries(raw.metadata).map(([key, value]) => {
                          if (excludedImageMetadataFields.includes(key))
                            return null;
                          return (
                            <tr key={key}>
                              <th className="c-data-list__key">
                        ...
                        {thumbnailFile && (
                          <li>
                            <a
                              className="dropdown-item"
                              target="_blank"
                              href={`${projectUrl}/${thumbnailFile.path}`}
                              rel="noreferrer"
                            >
                              View Thumbnail
                            </a>
                        ...
                        {histogramFile && (
                          <li>
                            <a
                              className="dropdown-item"
                              target="_blank"
                              href={`${projectUrl}/${histogramFile.path}`}
                              rel="noreferrer"
                            >
                              Histogram
                            </a>
                        ...
                        {histogramCsvFile && (
                          <li>
                            <a
                              className="dropdown-item"
                              target="_blank"
                              href={`${projectUrl}/${histogramCsvFile.path}`}
                              rel="noreferrer"
                            >
                              Histogram (CSV)
                            </a>
