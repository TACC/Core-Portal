...
// TODOV3: Duplicate code, but will be removed once we drop support for V2 jobs
function JobHistoryModal({ uuid, version }) {
  if (version === 'v3') {
    loading = useSelector((state) => state.jobDetail.loading);
    loadingError = useSelector((state) => state.jobDetail.loadingError);
    ...
    const job = useSelector((state) => {
      return state.jobsv2.list.find((job) => job.id === uuid);
    });
