...
describe('neurodataSaveComponent', function() {
    let ctrl,
        $q,
        file,
        scope,
    ...
    it('should save to destination',  () => {
        spyOn(ctrl.resolve.file, 'neurodataSave');
        ctrl.$onInit();
        ctrl.chooseDestination({'system': 'test.system', 'path': '/path/to/target'});
        expect(ctrl.resolve.file.neurodataSave).toHaveBeenCalledWith(
            {targetSystem: 'test.system', targetPath: '/path/to/target'} 
        );
    }) 
});
