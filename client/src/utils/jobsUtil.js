...
export function getJobDisplayInformation(job, app) {
  ...
  const schedulerOptions = parameterSet.schedulerOptions;
   ...
   /**
   * Strips trailing autogenerated Tapis suffixes like `_1.2` which 
   * Tapis adds to file names.
   *
   * Examples:
   * - "Input File_1.1" → "Input File"
   * - "_1.2" → "-----"   (only autogenerated part, no meaningful name)
   * - null / "" → "Unnamed Input"
   */
  function getCleanInputLabel(inputName) {
    const original = inputName || '';
    const stripped = original.replace(/_\d+\.\d+$/, '').trim();

    if (!stripped) {
      return /^_\d+\.\d+$/.test(original.trim()) ? '-----' : 'Unnamed Input';
    }
  ...
  const display = {
    ...
    inputs: fileInputs.map((input, index) => ({
      label: getCleanInputLabel(input.name, index),
      id: input.sourceUrl,
